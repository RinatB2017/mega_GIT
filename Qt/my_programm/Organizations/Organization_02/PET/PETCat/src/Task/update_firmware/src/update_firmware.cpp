...
#define NAME_UID            "UID"
#define NAME_DATE_FIRMWARE  "Версия платформы"
#define NAME_VER_FIRMWARE   "Версия ПО"
#define NAME_STATUS         "Состояние"

#define COL_DEVICE          0
#define COL_ADDR            1
#define COL_UID             2
#define COL_DATE_FIRMWARE   3
#define COL_VER_FIRMWARE    4
#define COL_STATUS          5
//--------------------------------------------------------------------------------
#define SET_GLASS
//--------------------------------------------------------------------------------
...
void UpdateFirmware::init(void)
{
    ...
    connect(comm,   SIGNAL(trace(QString)),     this,   SIGNAL(trace(QString)));
    //---
    ...
    connect(ui->btn_check_all,      SIGNAL(clicked(bool)),  this,   SLOT(btn_check_all_click()));
    connect(ui->btn_uncheck_all,    SIGNAL(clicked(bool)),  this,   SLOT(btn_uncheck_all_click()));
    //---
    block_wheel();
}
//--------------------------------------------------------------------------------
void UpdateFirmware::btn_refresh_click(void)
{
...
//--------------------------------------------------------------------------------
void UpdateFirmware::btn_check_all_click(void)
{
    for(int i=0; i<ui->tw_main->topLevelItemCount(); i++)
    {
        QTreeWidgetItem *item = ui->tw_main->topLevelItem(i);
        if(item)
        {
...
//--------------------------------------------------------------------------------
void UpdateFirmware::btn_uncheck_all_click(void)
{
    for(int i=0; i<ui->tw_main->topLevelItemCount(); i++)
    {
        QTreeWidgetItem *item = ui->tw_main->topLevelItem(i);
...
void UpdateFirmware::update_tree(void)
{
    ...
    emit debug(QString("Host [%1]").arg(host.toString()));
    //---
    ...
    foreach (pet_discovery_data_t d_packet, p_pet_config)
    {
        if(d_packet.pid == PET_PID_BRG)
        {
            ...
            //---
            QTreeWidgetItem *item = new QTreeWidgetItem(ui->tw_main, QTreeWidgetItem::Type);
            ...
            item->setTextAlignment(COL_UID,             Qt::AlignHCenter | Qt::AlignVCenter);
            ...
            //---
            item->setText(COL_DEVICE,           PET_device_base::get_device_type_short_str(d_packet.pid));
            item->setText(COL_ADDR,             QString("%1").arg(d_packet.addr));
            item->setText(COL_UID,              PET_device_base::get_PET_UID_string(d_packet.uid));
...
void UpdateFirmware::update(void)
{
    ...
    foreach (QTreeWidgetItem *item, uf_list)
    {
        ...
        {
            QString tmp_str = brg->get_PET_RESULT_string(res);
            emit error(tmp_str);
                                         ...
                                         QByteArray data)
{
    ...
    if(data.length() > (int)PET_FRAME_MAX)
    {
        emit error(QString("data.length() = %1").arg(data.length()));
        return PET_ERR_BAD_DATA;
    }
    ...
    packet.crc = CRC::pet_crc32((char *) &data.data()[0], data.length());

    QByteArray u_data;
    u_data.clear();
    u_data.append((char *)&packet, sizeof(packet));
    u_data.append(data);

    int s_data = u_data.length();
    int res = 0;
    int s_res = sizeof(res);

    brg->set_pet_addr(pet_address);
    return brg->update(u_data.data(),   &s_data,
                       &res,            &s_res);
}
//--------------------------------------------------------------------------------
bool UpdateFirmware::choice_firmware_brg(void)
{
    MyFileDialog *dlg = 0;
    QString temp_str;
    ...
    if(fi.size() == 0)
    {
        temp_str = QString("Файл %1 слишком мал. Размер %2 байтов")
                .arg(filename)
                .arg(fi.size());
        messagebox_critical("Ошибка", temp_str);
        return false;
