    ...
    MyWidget(parent)
{
    ...
    cb_symbol = new QCheckBox("symbol", this);

    //---
    cb_type_curve = new QComboBox();
    cb_type_curve->clear();
    cb_type_curve->addItem("Точечный", DOTS);
    cb_type_curve->addItem("Линейный", LINES);
    ...
    hbox->addWidget(cb_symbol);    
    hbox->addWidget(cb_type_curve);
    setLayout(hbox);
...
//--------------------------------------------------------------------------------
void GrapherCurveBox::set_symbol_state(bool state)
{
    cb_symbol->setChecked(state);
}
//--------------------------------------------------------------------------------
bool GrapherCurveBox::get_symbol_state(void)
{
    return cb_symbol->isChecked();
}
//--------------------------------------------------------------------------------
