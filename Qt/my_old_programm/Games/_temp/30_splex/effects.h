...
void Init_Flame()
 {
  load_LINEAR("./data/img/gfx/flame/flame.png",flame_texture);
 }
    ...
    if (flame_life[i]>0)
     {
      ...
      DrawRubberPoly(flame_X[i]+y_amp*sin(y_n*flame_Y[i]),
                     flame_Y[i],
                     flame_DX[i],
                     flame_DY[i],
                     flame_rotate[i],
...
//Эффекты
void Draw_Effects()//Рисуем эффекты
 {  
  for(int i=0; i<eff_num_max;i++)
   {
    if (eff_true[i]==true)
     {
      if  (eff_type[i]==index_dead_light)//молния
       {
      ...
      if  (eff_type[i]==index_dead_boom)//взрыв
       {
      ...
      if  (eff_type[i]==index_dead_blood)//кровища
       {
        ControlAnimate(&eff_frame[i],
                       1,
                       8,
