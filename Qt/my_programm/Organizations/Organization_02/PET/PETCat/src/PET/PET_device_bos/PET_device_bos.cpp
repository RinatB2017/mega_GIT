...
//--------------------------------------------------------------------------------
PET_device_bos::~PET_device_bos()
{

}
//--------------------------------------------------------------------------------
int PET_device_bos::get_PID(void)
{
    return PET_PID_BOS;
}
//--------------------------------------------------------------------------------
PET_RESULT PET_device_bos::test(void *i_data, int *i_data_len,
...
//--------------------------------------------------------------------------------
PET_RESULT PET_device_bos::get_device_info(pet_device_info_t *data,
                                           int *data_len)
{
    Q_CHECK_PTR(data);
...
//--------------------------------------------------------------------------------
PET_RESULT PET_device_bos::get_ctrl(pet_bos_ctrl_t *data,
                                    int *data_len)
{
    Q_CHECK_PTR(data);
    Q_CHECK_PTR(data_len);
...
//--------------------------------------------------------------------------------
PET_RESULT PET_device_bos::get_state(pet_bos_state_t *data,
                                     int *data_len)
{
    Q_CHECK_PTR(data);
    Q_CHECK_PTR(data_len);
...
//--------------------------------------------------------------------------------
PET_RESULT PET_device_bos::get_ch_ctrl(uint16_t *i_data, int *i_data_len,
...
//--------------------------------------------------------------------------------
PET_RESULT PET_device_bos::get_ch_state(uint16_t *i_data, int *i_data_len,
...
//--------------------------------------------------------------------------------
void PET_device_bos::f_import(void)
{
    QString filename;
    filename.clear();

    MyFileDialog *dlg = new MyFileDialog("filedialog_states", "bos_dlg");
...
//--------------------------------------------------------------------------------
void PET_device_bos::f_export(void)
{
    QString filename;
    filename.clear();
...
//--------------------------------------------------------------------------------
void PET_device_bos::save_to_EEPROM(void)
{
    ...
    if(res != PET_SUCCESS)
    {
        ...
        QString text = "Сохранение текущей конфигурации устройства в энергонезависимую память не удалась!";
...
//--------------------------------------------------------------------------------
void PET_device_bos::set_default(void)
{
    ...
    {
        QString title = "Ошибка";
        QString text = "Установка конфигурации устройства в значения по умолчанию не удалась!";
        messagebox_critical(title, text);
