...
void MyWidget::print_event_flag_log(void)
{
    ...
    float cnt_PET_EVENT_FLAG_FAULT_filtered_perc            = 0;
    float cnt_PET_EVENT_FLAG_TS_INVALID_filtered_perc       = 0;
    float cnt_PET_EVENT_FLAG_GATE_OVERLAY_filtered_perc     = 0;
    float cnt_PET_EVENT_FLAG_QDC_OVERFLOW_filtered_perc     = 0;
    float cnt_PET_EVENT_FLAG_QDC_UNDERFLOW_filtered_perc    = 0;
    float cnt_PET_EVENT_FLAG_ADC_OVERFLOW_filtered_perc     = 0;
    float cnt_PET_EVENT_FLAG_ADC_UNDERFLOW_filtered_perc    = 0;
    float cnt_PET_EVENT_FLAG_SYNC_filtered_perc             = 0;
    float all_filtered_perc = 0;

    //---
    if(bad_flags)
        cnt_PET_EVENT_FLAG_FAULT_filtered_perc            = (float)cnt_PET_EVENT_FLAG_FAULT_filtered / (float)bad_flags * 100.0f;
    if(bad_flags)
        cnt_PET_EVENT_FLAG_TS_INVALID_filtered_perc       = (float)cnt_PET_EVENT_FLAG_TS_INVALID_filtered / (float)bad_flags * 100.0f;
    if(bad_flags)
        cnt_PET_EVENT_FLAG_GATE_OVERLAY_filtered_perc     = (float)cnt_PET_EVENT_FLAG_GATE_OVERLAY_filtered / (float)bad_flags * 100.0f;
    if(bad_flags)
        cnt_PET_EVENT_FLAG_QDC_OVERFLOW_filtered_perc     = (float)cnt_PET_EVENT_FLAG_QDC_OVERFLOW_filtered / (float)bad_flags * 100.0f;
    if(bad_flags)
        cnt_PET_EVENT_FLAG_QDC_UNDERFLOW_filtered_perc    = (float)cnt_PET_EVENT_FLAG_QDC_UNDERFLOW_filtered / (float)bad_flags * 100.0f;
    if(bad_flags)
        cnt_PET_EVENT_FLAG_ADC_OVERFLOW_filtered_perc     = (float)cnt_PET_EVENT_FLAG_ADC_OVERFLOW_filtered / (float)bad_flags * 100.0f;
    if(bad_flags)
        cnt_PET_EVENT_FLAG_ADC_UNDERFLOW_filtered_perc    = (float)cnt_PET_EVENT_FLAG_ADC_UNDERFLOW_filtered / (float)bad_flags * 100.0f;
    if(bad_flags)
        cnt_PET_EVENT_FLAG_SYNC_filtered_perc             = (float)cnt_PET_EVENT_FLAG_SYNC_filtered / (float)bad_flags * 100.0f;

    if(cnt_events)
        all_filtered_perc = (float)all_filtered / (float)cnt_events * 100.0f;
    //---
    ...
    qDebug() << (float)all_filtered_perc << (float)all_filtered << cnt_events;
