...
void ViewerBox::init_widgets(void)
{
    ...
    connect(ui->btn_stat,   SIGNAL(clicked(bool)),  this,   SLOT(show_stat()));
}
//--------------------------------------------------------------------------------
...
void ViewerBox::show_stat(void)
{
    ...
    pet_event_t event;
    //---
    QProgressDialog progress("Подсчет статистики", "Отмена", 0, max_num_events, this);
    progress.setFixedWidth(MESSAGEBOX_WIDTH);
    progress.setWindowModality(Qt::WindowModal);
    progress.show();
    int inc = max_num_events / 100;
    ...
    file->close();
    //---
    QString temp = QString("Количество записей с нулевыми флагами = %1 (всего %2 событий)")
            .arg(cnt_zero)
            .arg(max_num_events);
    //---
    messagebox_info("Информация", temp);
...
void ViewerBox::show_event_dat3(void)
{
    ...
    if(filename.isEmpty())
    {
        temp_str = "Файл данных не выбран!";
        emit error(temp_str);
        messagebox_critical("Ошибка", temp_str);
        return;
    }
