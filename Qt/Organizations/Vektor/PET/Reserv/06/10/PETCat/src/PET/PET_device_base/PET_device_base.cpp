...
PET_RESULT PET_device_base::reset(void)
{
    ...
    err = send_packet(question, &answer);
    if(err != PET_SUCCESS)
    {
        return err;
    }
...
#if 0   //TODO
    pet_frame_t *i_frame = (pet_frame_t *)answer.data();
    if(i_frame->len != 0)
    {
...
#if 0   //TODO
    if(r_frame->len != ((int)sizeof(pet_update_data_t) + i_data->length))
    {
        return PET_ERR_BAD_DATA;
    }
#endif
                                             ...
                                             void *o_data, int *o_data_len)
{
    ...
    pet_frame_t *r_frame = (pet_frame_t *)answer.data();
    int len = answer.length() - sizeof(frame);
    if(len != sizeof(pet_brg_calibration_t))
    {
        return PET_ERR_BAD_DATA;
    }
    //---
