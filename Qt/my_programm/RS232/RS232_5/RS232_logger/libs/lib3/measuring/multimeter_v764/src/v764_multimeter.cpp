...
#include "defines.hpp"
//--------------------------------------------------------------------------------
Multimeter_V764::Multimeter_V764(QWidget *parent) :
    MyWidget(parent),
    delay_ms(1000),
    address(1)
{
...
Multimeter_V764::~Multimeter_V764()
{
    ...
    if(serial)
    {
        ...
        disconnect(serial, &SerialBox5_wo_form::output,
                   this,   &Multimeter_V764::port_read);
#else
        disconnect(serial, &SerialPort_V764_test::output,
                   this,   &Multimeter_V764::port_read);
#endif
        delete serial;
    }
...
void Multimeter_V764::init(void)
{
    ...
    connect(serial, &SerialBox5_wo_form::info,  this,   &Multimeter_V764::info);
    connect(serial, &SerialBox5_wo_form::debug, this,   &Multimeter_V764::debug);
    connect(serial, &SerialBox5_wo_form::error, this,   &Multimeter_V764::error);
    connect(serial, &SerialBox5_wo_form::trace, this,   &Multimeter_V764::trace);

    connect(serial, &SerialBox5_wo_form::port_is_active,    this,   &Multimeter_V764::port_is_active);
    connect(serial, &SerialBox5_wo_form::output,            this,   &Multimeter_V764::port_read);
#else
    serial = new SerialPort_V764_test();

    connect(serial, &SerialPort_V764_test::info,  this,   &Multimeter_V764::info);
    connect(serial, &SerialPort_V764_test::debug, this,   &Multimeter_V764::debug);
    connect(serial, &SerialPort_V764_test::error, this,   &Multimeter_V764::error);
    connect(serial, &SerialPort_V764_test::trace, this,   &Multimeter_V764::trace);

    connect(serial, &SerialPort_V764_test::output,
            this,   &Multimeter_V764::port_read);

    //TODO для синусоиды
    connect(this,   &Multimeter_V764::get_test_value,
            serial, &SerialPort_V764_test::get_test_value);
#endif
}
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
void Multimeter_V764::port_read(const QByteArray &data)
{
...
int Multimeter_V764::send_cmd_0x47(int *value_uV)
{
    ...
    *value_uV = static_cast<int>(value * 1e6);
    emit trace(QString("Значение: %1").arg(value, 0, 'f', 6));
    data_is_ready = false;
...
int Multimeter_V764::print_last_error(void)
{
    ...
    case E_V764_ERROR_SIZE_NOT_CORRECT:   emit error(tr("Длина данных не соответствует завленной")); break;
...
//--------------------------------------------------------------------------------
bool Multimeter_V764::port_is_open(void)
{
...
#endif
//--------------------------------------------------------------------------------
void Multimeter_V764::wait_msec(int timeout_msec)
{
    QElapsedTimer time;
...
//--------------------------------------------------------------------------------
void Multimeter_V764::updateText(void)
{

}
//--------------------------------------------------------------------------------
bool Multimeter_V764::programm_is_exit(void)
{
    return true;
}
//--------------------------------------------------------------------------------
void Multimeter_V764::load_setting(void)
{

}
//--------------------------------------------------------------------------------
void Multimeter_V764::save_setting(void)
{
