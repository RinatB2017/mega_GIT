...
void GrapherBox::create_widgets(void)
{
    ...
    connect(ui->btn_all_on,     SIGNAL(clicked(bool)),  this, SLOT(btn_all_on_click()));
    connect(ui->btn_all_off,    SIGNAL(clicked(bool)),  this, SLOT(btn_all_off_click()));

    connect(ui->btn_Options,    SIGNAL(clicked()), this, SLOT(btn_Options_click()));
    connect(ui->btn_Clear,      SIGNAL(clicked()), this, SLOT(btn_Clear_click()));
    connect(ui->btn_Load,       SIGNAL(clicked()), this, SLOT(btn_Load_click()));
    connect(ui->btn_Save,       SIGNAL(clicked()), this, SLOT(btn_Save_click()));

    connect(ui->btn_Horizontal, SIGNAL(toggled(bool)), this, SLOT(btn_Horizontal_click(bool)));
    connect(ui->btn_Vertical,   SIGNAL(toggled(bool)), this, SLOT(btn_Vertical_click(bool)));

    connect(ui->btn_Statistic,  SIGNAL(clicked()), this, SLOT(btn_Statistic_click()));
...
//--------------------------------------------------------------------------------
void GrapherBox::btn_all_on_click(void)
{
    legends_all_on();
}
//--------------------------------------------------------------------------------
void GrapherBox::btn_all_off_click(void)
{
     legends_all_off();
}
//--------------------------------------------------------------------------------
void GrapherBox::btn_Options_click(void)
{
    options();
}
//--------------------------------------------------------------------------------
void GrapherBox::btn_Clear_click(void)
{
    clear();
}
//--------------------------------------------------------------------------------
void GrapherBox::btn_Load_click(void)
{
    load_curves();
}
//--------------------------------------------------------------------------------
void GrapherBox::btn_Save_click(void)
{
    save_curves();
}
//--------------------------------------------------------------------------------
void GrapherBox::btn_Horizontal_click(bool state)
{
    tune_horizontal_axis(state);
}
//--------------------------------------------------------------------------------
void GrapherBox::btn_Vertical_click(bool state)
{
    tune_vertical_axis(state);
}
//--------------------------------------------------------------------------------
void GrapherBox::btn_Statistic_click(void)
{
    statistic();
}
//--------------------------------------------------------------------------------
...
void GrapherBox::popup(QPoint)
{
    ...
    QAction *setting_action = new QAction(qApp->style()->standardIcon(QStyle::SP_ComputerIcon),
                                          ...
                                          this);
    //QAction *curve_correct_action = new QAction(qApp->style()->standardIcon(QStyle::SP_ComputerIcon),
    //                                            QObject::tr("корректировка данных"),
    //                                            this);

    popup_menu->addAction(setting_action);
    //popup_menu->addAction(curve_correct_action);
    //popup_menu->addSeparator();

    connect(setting_action,  SIGNAL(triggered()), this, SLOT(slot_setting_action()));
    //connect(curve_correct_action, SIGNAL(triggered()), this, SLOT(slot_correct_action()));

    popup_menu->exec(QCursor::pos());
}
//--------------------------------------------------------------------------------
void GrapherBox::slot_correct_action(void)
{
    CurveCorrects *dlg = new CurveCorrects(curves, this);
    int button = dlg->exec();
...
//--------------------------------------------------------------------------------
void GrapherBox::slot_setting_action(void)
{
    GrapherSettings *dlg = new GrapherSettings(curves, d_picker, this);
    int button = dlg->exec();
    if(button == QDialog::Accepted)
    {
        ...
        foreach (GRAPHER_CURVE curve, curves)
        {
            bool symbol_state = dlg->get_symbol_state(n);
            if(symbol_state)
            {
                if(curve.symbol_is_active)
                {
                    QwtSymbol *symbol = new QwtSymbol(curve.symbol_style,
                                                      curve.symbol_brush,
                                                      curve.symbol_pen,
                                                      curve.symbol_size);
                    curve.plot_curve->setSymbol(symbol);
            ...
            else
            {
                ...
                curve.plot_curve->setSymbol(nosymbol);
            }
            ...
            int g = 0;
            int b = 0;
            dlg->get_RGB(n, &r, &g, &b, &ok);
            if(ok)
            {
                curve.plot_curve->setPen(QColor(r, g, b));
                bool ok = false;
                QString title = dlg->get_caption(n, &ok);
                if(ok)
                {
                    curve.plot_curve->setTitle(title);
                }
                updateGraphics();
            }
            n++;
...
void GrapherBox::load_curves(void)
{
    ...
    dlg = new MyFileDialog("filedialog_states", "gb_dlg");
...
void GrapherBox::save_curves(void)
{
    ...
    MyFileDialog *dlg = new MyFileDialog("filedialog_states", "gb_dlg");
