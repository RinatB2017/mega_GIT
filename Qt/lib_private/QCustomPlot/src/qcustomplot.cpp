...
void QCPItemBracket::draw(QCPPainter *painter)
{
    ...
    if (clip.intersects(boundingPoly.boundingRect()))
    {
        ...
        case bsCalligraphic:
        {
            ...
            path.cubicTo((centerVec-0.4*widthVec+0.8f*lengthVec).toPointF(), (centerVec-widthVec*1-lengthVec*0.8f).toPointF(), (centerVec-widthVec+lengthVec).toPointF());

            path.cubicTo((centerVec-widthVec*1-lengthVec*0.5f).toPointF(), (centerVec-0.2*widthVec+1.2f*lengthVec).toPointF(), (centerVec+lengthVec*0.2f).toPointF());
