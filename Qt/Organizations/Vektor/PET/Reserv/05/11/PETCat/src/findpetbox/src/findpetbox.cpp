...
#include "pet.h"
//--------------------------------------------------------------------------------
FindPetBox::FindPetBox(uint8_t addr,
                       QWidget *parent) :
    QDialog(parent),
...
void FindPetBox::init(void)
{
    ...
    connect(pet,    SIGNAL(message(QString)),   this,   SIGNAL(message(QString)));
    ...
    //---
    pet->set_comm_param(addr, PETCat_options::get_port_cmd());
...
bool FindPetBox::add_pet_device(pet_discovery_data p_device)
{
    ...
    {
        if(p_device.addr != PET_ADDR_ROOT)
        {
            return false;
        }
    }
