...
void FloodMapBox::refresh(void)
{
    ...
    emit info(QString("    Файл данных:                        %1")
              .arg(filename));
    emit info(QString("    размер изображения:                 %1")
              .arg(floodmap_size));
    emit info(QString("Всего событий:                          %1")
              .arg(cnt_events));
    emit info(QString("Отфильтровано:"));
    emit info(QString("    всего:                              %1")
              .arg(all_filtered));
    emit info(QString("    по адресу:                          %1")
              .arg((float)cnt_events_address_filtered / (float)cnt_events * 100.0f));
    emit info(QString("    по мин. энергии:                    %1")
              .arg((float)cnt_events_min_energy_filtered / (float)cnt_events * 100.0f));
    emit info(QString("    по макс. энергии:                   %1")
