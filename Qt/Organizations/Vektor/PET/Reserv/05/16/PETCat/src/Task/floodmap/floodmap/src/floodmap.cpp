...
void FloodMapBox::get_event(pet_event event)
{
    ...
    if(check_event_flags(event_flags, event.flags) == false)
    {
        return;
    }
...
bool FloodMapBox::check_event_flags(uint8_t filter_flags, uint8_t event_flags)
{
    ...
    //FIXME
    if(PET_EVENT_FLAG_FAULT & event_flags)
    {
        cnt_PET_EVENT_FLAG_FAULT_filtered++;
        bad_flags++;
        return ok;
    }
    if(PET_EVENT_FLAG_TS_INVALID & event_flags)
    {
        cnt_PET_EVENT_FLAG_TS_INVALID_filtered++;
        bad_flags++;
        return ok;
    }
    if(PET_EVENT_FLAG_GATE_OVERLAY & event_flags)
    {
        cnt_PET_EVENT_FLAG_GATE_OVERLAY_filtered++;
        bad_flags++;
        return ok;
    }
    if(PET_EVENT_FLAG_QDC_OVERFLOW & event_flags)
    {
        cnt_PET_EVENT_FLAG_QDC_OVERFLOW_filtered++;
        bad_flags++;
        return ok;
    }
    if(PET_EVENT_FLAG_QDC_UNDERFLOW & event_flags)
    {
        cnt_PET_EVENT_FLAG_QDC_UNDERFLOW_filtered++;
        bad_flags++;
        return ok;
    }
    if(PET_EVENT_FLAG_ADC_OVERFLOW & event_flags)
    {
        cnt_PET_EVENT_FLAG_ADC_OVERFLOW_filtered++;
        bad_flags++;
        return ok;
    }
    if(PET_EVENT_FLAG_ADC_UNDERFLOW & event_flags)
    {
        cnt_PET_EVENT_FLAG_ADC_UNDERFLOW_filtered++;
        bad_flags++;
        return ok;
    }
    if(PET_EVENT_FLAG_SYNC & event_flags)
    {
...
void FloodMapBox::refresh(void)
{
    ...
    if(!ok)
    {
        ...
        ui->sb_filter_address->setDisabled(ui->rb_event_all->isChecked());
        ui->btn_save_picture_to_file->setDisabled(true);
        ui->btn_save_picture_to_clipboard->setDisabled(true);
        return;
    }
    ...
    //TODO
    //scene->addRect(0, 0, image->width(), image->height(), QPen(Qt::green));
