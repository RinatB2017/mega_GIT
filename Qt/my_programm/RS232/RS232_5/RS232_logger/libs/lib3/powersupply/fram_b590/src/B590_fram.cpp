...
bool B590_Fram::read_fram_from_default_data(void)
{
    ...
    ok = B590_Fram::set_Year_Factory(2023);
    if(!ok) return false;
                                  ...
                                  s_point point)
{
    ...
    PROFILE.profile.point[PROFILE.profile.points_in_profile].hour    = point.hour;
    PROFILE.profile.point[PROFILE.profile.points_in_profile].min    = point.min;
    PROFILE.profile.point[PROFILE.profile.points_in_profile].sec    = point.sec;
