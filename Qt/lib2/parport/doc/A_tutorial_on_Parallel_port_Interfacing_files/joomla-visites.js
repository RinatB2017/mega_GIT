// http://www.joomla-visites.net/ 
// License GNU/GPL (http://www.gnu.org/copyleft/gpl.html)
function pmv_log(pmv_site, pmv_url, pmv_pname, pmv_vars, pmv_user)
{
	...
	if (!pmv_win || pmv_moz){
		...
		if (plugIE("PDF.PdfCtrl.1") == '1' || plugIE('PDF.PdfCtrl.5') == '1' || plugIE('PDF.PdfCtrl.6') == '1') 
			pmv_pdf = '1';
		pmv_qt = plugIE("Quicktime.Quicktime"); // Old : "QuickTimeCheckObject.QuickTimeCheck.1"
		pmv_rea = plugIE("rmocx.RealPlayer G2 Control.1");
		pmv_wma = plugIE("wmplayer.ocx"); // Old : "MediaPlayer.MediaPlayer.1"
	}
	...
	try {rtu = top.pmv_do.referrer;} catch(e) {
		try {rtu = pmv_do.referrer;} catch(E) {
			try {rtu = Xt_r;} catch(E) {rtu = '';}
		}
	}
	...
	src += '&pdf='+pmv_pdf+'&windowsmedia='+pmv_wma+'&java='+pmv_jav+'&ref='+escape(rtu)+'&user='+pmv_user;

	pmv_do.writeln('<img src="'+src+'" alt="Joomla-Visites" style="border:0" />');
}
pmv_log(joomlaVisitesSite, joomlaVisitesURL, pagename, a_vars, joomlaVisitesUser);