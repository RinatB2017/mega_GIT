...
//--------------------------------------------------------------------------------
Powersupply_B590::Powersupply_B590(QWidget *parent) :
    MyWidget(parent),
    is_ready(false),
    busy(false),
    address(1),
...
bool Powersupply_B590::find_device(void)
{
    ...
    emit info(QString("Найдено портов: %1").arg(ports.count()));
    foreach (const QSerialPortInfo &port, ports)
    {
        emit info(QString("ищем источник питания на порту %1").arg(port.portName()));
        foreach (int speed, speeds)
        {
            ...
            if(ok)
            {
                ...
                if(serial.open(QIODevice::ReadWrite))
                {
                    ...
                    {
                        emit info("Проверяем идентификатор источника");
                        if(type == 4)
                        {
                            emit info(QString("Источник питания найден на скорости %1").arg(speed));
                            return true;
                        }
                        else
                        {
...
{
    emit debug(QString("new delay %1 ms").arg(delay_ms));
...
int Powersupply_B590::check_dirty_packet(const QByteArray &data)
{
    ...
    uint8_t cmd = data.at(1) | 0x7F;
    //uint8_t err = data.at(1) | 0x80;
...
int Powersupply_B590::send_cmd_0x11(void)
{
    ...
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_11_QUESTION) - 2);
...
int Powersupply_B590::send_cmd_0x12(void)
{
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_12_QUESTION) - 2);
...
int Powersupply_B590::send_cmd_0x15(void)
{
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_15_QUESTION) - 2);
...
int Powersupply_B590::send_cmd_0x16(void)
{
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_16_QUESTION) - 2);
...
int Powersupply_B590::send_cmd_0x17(void)
{
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_17_QUESTION) - 2);
...
int Powersupply_B590::send_cmd_0x41(void)
{
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_41_QUESTION) - 2);
...
//--------------------------------------------------------------------------------
int Powersupply_B590::send_cmd_0x42(uint8_t code_speed)
{
    for(int n=0; n<3; n++)
    {
        if(busy) wait(delay_ms);
    ...
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_42_QUESTION) - 2);
...
//--------------------------------------------------------------------------------
int Powersupply_B590::check_packet_answer(uint8_t cmd,
                                          int packet_len)
{
    ...
    if(!ignore_bad_cmd)
    {
        uint8_t temp = data_powersupply.at(1) & 0x80;
        uint8_t data_cmd = data_powersupply.at(1) & 0x7F;
        if(data_cmd != cmd)
        {
    ...
    uint16_t calc_crc16 = CRC::powersupply_crc16((uint8_t *)data_powersupply.constData(),
                                                       packet_len - 2);
                                    ...
                                    uint16_t *serno)
{
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_46_QUESTION) - 2);
                                    ...
                                    bool *flag_zummer)
{
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_47_QUESTION) - 2);
    ...
    //    B590_CMD_47_ANSWER answer = data_powersupply.data();
...
//--------------------------------------------------------------------------------
int Powersupply_B590::send_cmd_0x48(uint8_t new_address_MODBUS,
                                    uint8_t *r_new_address_MODBUS)
{
    ...
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_48_QUESTION) - 2);
                                    ...
                                    uint16_t current)
{
    ...
    {
        last_error = E_B590_BUSY;
        emit error("Не прошла уставка, занято");
        return last_error;
    }
    ...
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_49_QUESTION) - 2);
                                    ...
                                    uint16_t *code)
{
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_4A_QUESTION) - 2);
...
//--------------------------------------------------------------------------------
int Powersupply_B590::send_cmd_0x54(uint8_t profile,
                                    uint8_t *count_point,
                                    uint8_t *count_repeat_profile)
{
    ...
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_54_QUESTION) - 2);
...
int Powersupply_B590::send_cmd_0x55(unsigned int *mototime_min)
{
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_55_QUESTION) - 2);
                                    ...
                                    uint16_t *r_time)
{
    ...
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_56_QUESTION) - 2);
                                    ...
                                    uint16_t I)
{
    ...
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_57_QUESTION) - 2);
                                    ...
                                    uint8_t  *point)
{
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_5A_QUESTION) - 2);
                                    ...
                                    uint32_t *U_input)
{
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_5B_QUESTION) - 2);
                                    ...
                                    uint8_t       *r_error)
{
    ...
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_5C_QUESTION) - 2);
                                    ...
                                    s_zero_b590_I *zero_I)
{
    ...
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_5D_QUESTION) - 2);
                                    ...
                                    uint16_t *r_time)
{
    ...
    question.body.data.sec = time;

    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_5E_QUESTION) - 2);
...
//--------------------------------------------------------------------------------
int Powersupply_B590::send_cmd_0x5F(uint8_t profile,
                                    uint8_t *number_profile)
{
    ...
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_5F_QUESTION) - 2);
...
int Powersupply_B590::send_cmd_0x60(void)
{
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_60_QUESTION) - 2);
                                    ...
                                    double  *factor_k_ADC)
{
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_61_QUESTION) - 2);
                                    ...
                                    double  *factor_k_ADC)
{
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_62_QUESTION) - 2);
                                    ...
                                    uint16_t *r_data_point)
{
    ...
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_63_QUESTION) - 2);
...
int Powersupply_B590::send_cmd_0x64(char code)
{
    ...
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_64_QUESTION) - 2);
                                    ...
                                    uint8_t *state)
{
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_65_QUESTION) - 2);
                                    ...
                                    uint16_t *value)
{
    ...
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_66_QUESTION) - 2);
...
//--------------------------------------------------------------------------------
int Powersupply_B590::send_cmd_0x67(uint16_t speed_cooler,
                                    uint16_t *r_speed_cooler)
{
    ...
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_67_QUESTION) - 2);
...
//--------------------------------------------------------------------------------
int Powersupply_B590::send_cmd_0x68(uint16_t data_PWM,
                                    uint16_t *r_data_PWM)
{
    for(int n=0; n<3; n++)
    {
    ...
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_68_QUESTION) - 2);
...
//--------------------------------------------------------------------------------
int Powersupply_B590::send_cmd_0x69(uint16_t *OCR3A,
                                    uint16_t *OCR3B)
{
    for(int n=0; n<3; n++)
    {
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_69_QUESTION) - 2);
...
int Powersupply_B590::send_cmd_0x6A(void)
{
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_6A_QUESTION) - 2);
    ...
    if(data_byte.bites.bit0) emit info("Захват произведен.");
    if(data_byte.bites.bit1) emit error("Захват невозможен, т.к. ручное управление.");
    if(data_byte.bites.bit2) emit error("Захват невозможен, т.к. RS232 сейчас используется.");
    if(data_byte.bites.bit4) emit error("Захват невозможен, т.к. USB сейчас используется.");
...
int Powersupply_B590::send_cmd_0x6B(void)
{
    ...
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_6B_QUESTION) - 2);
    ...
    U_BYTE data_byte;
    data_byte.value = answer->body.data.current_driver;
    if(data_byte.bites.bit0) emit info("Снятие произведено.");
    if(data_byte.bites.bit1) emit error("Снятие невозможено, т.к. ручное управление.");
    if(data_byte.bites.bit2) emit error("Снятие невозможено, т.к. RS232 сейчас используется.");
    if(data_byte.bites.bit4) emit error("Снятие невозможено, т.к. USB сейчас используется.");
...
int Powersupply_B590::send_cmd_0x6C(unsigned int number_string)
{
    ...
    if(number_string > 2)
    {
        emit error(QString("Недопустимый номер строки %1").arg(number_string));
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_6C_QUESTION) - 2);
    ...
    case 2:
        ...
        */
        ...
        uint16_t version_comp = temp_version_comp.value;
        uint16_t version_proc = temp_version_proc.value;
                                    ...
                                    QByteArray *output_data)
{
    ...
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_6D_QUESTION) - 2);
...
//--------------------------------------------------------------------------------
int Powersupply_B590::send_cmd_0x6E(uint8_t profile,
                                    uint8_t repeat,
                                    uint8_t *r_profile,
                                    uint8_t *r_repeat)
{
    ...
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_6E_QUESTION) - 2);
...
//--------------------------------------------------------------------------------
int Powersupply_B590::send_cmd_0x6F(uint8_t profile,
                                    uint8_t *r_profile)
{
    for(int n=0; n<3; n++)
    {
    ...
    question.body.header.count_data = sizeof(question.body.data);
    question.body.data.profile = profile;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_6F_QUESTION) - 2);
...
//--------------------------------------------------------------------------------
int Powersupply_B590::send_cmd_0x71(uint8_t *state_ADC_U,
                                    uint8_t *state_ADC_I,
                                    uint8_t *state_ADC_Ui)
{
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_71_QUESTION) - 2);
...
//--------------------------------------------------------------------------------
int Powersupply_B590::send_cmd_0x72(uint16_t conf_ADC,
                                    uint16_t *r_conf_ADC)
{
    for(int n=0; n<3; n++)
    {
    ...
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_72_QUESTION) - 2);
...
//--------------------------------------------------------------------------------
int Powersupply_B590::send_cmd_0x73(uint16_t *real_DAC_U,
                                    uint16_t *real_DAC_I,
                                    uint8_t  *revers,
                                    uint16_t *temperature)
{
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_73_QUESTION) - 2);
...
//--------------------------------------------------------------------------------
int Powersupply_B590::send_cmd_0x75(uint8_t *count_ReStart_ADC_1,
                                    ...
                                    uint16_t *MCUCSR)
{
    ...
    question.body.header.count_data = 0;
    question.body.crc16 = CRC::powersupply_crc16((uint8_t *)&question.buf, sizeof(B590_CMD_75_QUESTION) - 2);
...
//--------------------------------------------------------------------------------
bool Powersupply_B590::get_temperature(uint8_t *temp,
                                       bool *flag_zummer)
{
...
{
    uint16_t real_DAC_U = 0;
    uint16_t real_DAC_I = 0;
    uint8_t  revers = 0;
                              ...
                              uint16_t current_mA)
{
    ...
    if(((uint32_t)abs(voltage_mV)*current_mA/1000000)>750)
    {
        QMessageBox mes;
        mes.setWindowTitle("Ошибка");
        mes.setText("Превышено допустимое значение мощности(750Вт)");
...
bool Powersupply_B590::set_UI_parrot(uint16_t voltage,
                                     uint16_t current)
{
    emit info(QString("Выставляем значение DAC в квантах: u=%1, i=%2")
               .arg(voltage)
               .arg(current));
...
//--------------------------------------------------------------------------------
bool Powersupply_B590::set_vent_speed(uint16_t speed)
{
    uint16_t temp = 0;
...
//--------------------------------------------------------------------------------
bool Powersupply_B590::get_meas_UI(int32_t  *out_voltage,
                                   uint16_t *out_current,
                                   int8_t   *temp,
                                   uint8_t *current_point,
                                   uint8_t  *active_profile)
{
    int32_t  *in_voltage = nullptr;
    uint16_t *in_current = nullptr;
    bool flag_zummer = false;
    int err = send_cmd_0x47(in_voltage,
                            in_current,
                            out_voltage,
...
//--------------------------------------------------------------------------------
bool Powersupply_B590::Save_Calib_Point_DAC(uint8_t number_point,
                                            uint16_t data_point)
{
    uint8_t  r_number_point = 0;
    uint16_t r_data_point = 0;
    int err = send_cmd_0x63(0,
                            number_point,
                            data_point,
                            &r_number_point,
...
//--------------------------------------------------------------------------------
bool Powersupply_B590::Test_Calib_Point_DAC(uint8_t number_point)
{
    uint16_t value = 0;
...
{
    uint16_t speed_cooler = 0;
    uint16_t r_speed_cooler = 0;
...
void Powersupply_B590::run_dialog_0x42(void)
{
    ...
    grid->addWidget(new QLabel("Скорость"), 0, 0);
...
void Powersupply_B590::run_dialog_0x47(void)
{
    ...
    emit info(QString("active profile: %1").arg(active_profile));
    emit info(QString("flag_zummer: %1").arg(flag_zummer));
    emit info("OK");
}
//--------------------------------------------------------------------------------
void Powersupply_B590::run_dialog_0x48(void)
{
    ...
    QGridLayout *grid = new QGridLayout();
    grid->addWidget(new QLabel("Новый адрес MODBUS"),   0, 0);
    ...
    if(btn == QDialog::Accepted)
    {
        uint8_t r_new_address_MODBUS = 0;
...
void Powersupply_B590::run_dialog_0x49(void)
{
    ...
    QGridLayout *grid = new QGridLayout();
    grid->addWidget(new QLabel("Напряжение"),   0, 0);
    grid->addWidget(sb_voltage,                 0, 1);
    grid->addWidget(new QLabel("Ток"),          1, 0);
...
void Powersupply_B590::run_dialog_0x54(void)
{
    ...
    QGridLayout *grid = new QGridLayout();
    grid->addWidget(new QLabel("Профиль"),  0, 0);
...
void Powersupply_B590::run_dialog_0x56(void)
{
    ...
    grid->addWidget(new QLabel("Профиль"),   0, 0);
    grid->addWidget(sb_profile,              0, 1);
    grid->addWidget(new QLabel("Точка"),     1, 0);
...
void Powersupply_B590::run_dialog_0x57(void)
{
    ...
    QGridLayout *grid = new QGridLayout();
    grid->addWidget(new QLabel("Напряжение (0-65535)"), 0, 0);
    grid->addWidget(sb_U,                               0, 1);
    grid->addWidget(new QLabel("Ток (0-65535)"),        1, 0);
...
void Powersupply_B590::run_dialog_0x5C(void)
{
    ...
    QGridLayout *grid = new QGridLayout();
    grid->addWidget(new QLabel("Value_ADC_U_0"),            0, 0); grid->addWidget(sb_Value_ADC_U_0,             0, 1);
    grid->addWidget(new QLabel("Value_ADC_U_1"),            1, 0); grid->addWidget(sb_Value_ADC_U_1,             1, 1);
    grid->addWidget(new QLabel("Value_ADC_U_2"),            2, 0); grid->addWidget(sb_Value_ADC_U_2,             2, 1);
    grid->addWidget(new QLabel("Value_ADC_U_3"),            3, 0); grid->addWidget(sb_Value_ADC_U_3,             3, 1);

    grid->addWidget(new QLabel("Value_DAC_U_0"),            4, 0); grid->addWidget(sb_Value_DAC_U_0,             4, 1);
    grid->addWidget(new QLabel("Value_DAC_U_1"),            5, 0); grid->addWidget(sb_Value_DAC_U_1,             5, 1);
    grid->addWidget(new QLabel("Value_DAC_U_2"),            6, 0); grid->addWidget(sb_Value_DAC_U_2,             6, 1);
    grid->addWidget(new QLabel("Value_DAC_U_3"),            7, 0); grid->addWidget(sb_Value_DAC_U_3,             7, 1);

    grid->addWidget(new QLabel("parrot_in_point"),          8, 0); grid->addWidget(sb_parrot_in_point,           8, 1);

    //---
    grid->addWidget(new QLabel("Value_ADC_I_0"),            0, 2); grid->addWidget(sb_Value_ADC_I_0,             0, 3);
    grid->addWidget(new QLabel("Value_ADC_I_1"),            1, 2); grid->addWidget(sb_Value_ADC_I_1,             1, 3);
    grid->addWidget(new QLabel("Value_ADC_I_2"),            2, 2); grid->addWidget(sb_Value_ADC_I_2,             2, 3);
    grid->addWidget(new QLabel("Value_ADC_I_3"),            3, 2); grid->addWidget(sb_Value_ADC_I_3,             3, 3);
    grid->addWidget(new QLabel("Value_ADC_I_4"),            4, 2); grid->addWidget(sb_Value_ADC_I_4,             4, 3);
    grid->addWidget(new QLabel("Value_ADC_I_5"),            5, 2); grid->addWidget(sb_Value_ADC_I_5,             5, 3);
    grid->addWidget(new QLabel("Value_ADC_I_6"),            6, 2); grid->addWidget(sb_Value_ADC_I_6,             6, 3);
    grid->addWidget(new QLabel("Value_ADC_I_7"),            7, 2); grid->addWidget(sb_Value_ADC_I_7,             7, 3);
    grid->addWidget(new QLabel("Value_ADC_I_8"),            8, 2); grid->addWidget(sb_Value_ADC_I_8,             8, 3);
    grid->addWidget(new QLabel("Value_ADC_I_9"),            9, 2); grid->addWidget(sb_Value_ADC_I_9,             9, 3);
    grid->addWidget(new QLabel("Value_ADC_I_10"),          10, 2); grid->addWidget(sb_Value_ADC_I_10,           10, 3);
    grid->addWidget(new QLabel("Value_ADC_I_11"),          11, 2); grid->addWidget(sb_Value_ADC_I_11,           11, 3);
    grid->addWidget(new QLabel("Value_ADC_I_11_HOT"),      12, 2); grid->addWidget(sb_Value_ADC_I_11_HOT,       12, 3);
    grid->addWidget(new QLabel("Value_ADC_I_12"),          13, 2); grid->addWidget(sb_Value_ADC_I_12,           13, 3);

    grid->addWidget(new QLabel("Value_DAC_I_0"),            0, 6); grid->addWidget(sb_Value_DAC_I_0,             0, 7);
    grid->addWidget(new QLabel("Value_DAC_I_1"),            1, 6); grid->addWidget(sb_Value_DAC_I_1,             1, 7);
    grid->addWidget(new QLabel("Value_DAC_I_2"),            2, 6); grid->addWidget(sb_Value_DAC_I_2,             2, 7);
    grid->addWidget(new QLabel("Value_DAC_I_3"),            3, 6); grid->addWidget(sb_Value_DAC_I_3,             3, 7);
    grid->addWidget(new QLabel("Value_DAC_I_4"),            4, 6); grid->addWidget(sb_Value_DAC_I_4,             4, 7);
    grid->addWidget(new QLabel("Value_DAC_I_5"),            5, 6); grid->addWidget(sb_Value_DAC_I_5,             5, 7);
    grid->addWidget(new QLabel("Value_DAC_I_6"),            6, 6); grid->addWidget(sb_Value_DAC_I_6,             6, 7);
    grid->addWidget(new QLabel("Value_DAC_I_7"),            7, 6); grid->addWidget(sb_Value_DAC_I_7,             7, 7);
    grid->addWidget(new QLabel("Value_DAC_I_8"),            8, 6); grid->addWidget(sb_Value_DAC_I_8,             8, 7);
    grid->addWidget(new QLabel("Value_DAC_I_9"),            9, 6); grid->addWidget(sb_Value_DAC_I_9,             9, 7);
    grid->addWidget(new QLabel("Value_DAC_I_10"),          10, 6); grid->addWidget(sb_Value_DAC_I_10,           10, 7);
    grid->addWidget(new QLabel("Value_DAC_I_11"),          11, 6); grid->addWidget(sb_Value_DAC_I_11,           11, 7);
    grid->addWidget(new QLabel("Value_DAC_I_11_HOT"),      12, 6); grid->addWidget(sb_Value_DAC_I_11_HOT,       12, 7);
    grid->addWidget(new QLabel("Value_DAC_I_12"),          13, 6); grid->addWidget(sb_Value_DAC_I_12,           13, 7);
...
void Powersupply_B590::run_dialog_0x5E(void)
{
    ...
    grid->addWidget(new QLabel("Профиль"),      0, 0);
    grid->addWidget(sb_profile,                 0, 1);
    grid->addWidget(new QLabel("Напряжение"),   1, 0);
    grid->addWidget(sb_voltage_mV,              1, 1);
    grid->addWidget(new QLabel("Ток"),          2, 0);
    grid->addWidget(sb_current_mA,              3, 1);
    grid->addWidget(new QLabel("Время"),        3, 0);
...
//--------------------------------------------------------------------------------
bool Powersupply_B590::run_dialog_0x60(void)
{
...
void Powersupply_B590::run_dialog_0x63(void)
{
    ...
    QGridLayout *grid = new QGridLayout();
    grid->addWidget(new QLabel("Number point"), 0, 0);
    grid->addWidget(sb_number_point,            0, 1);
    grid->addWidget(new QLabel("Data point"),   1, 0);
...
void Powersupply_B590::run_dialog_0x64(void)
{
    ...
    grid->addWidget(new QLabel("number"), 5, 0);
...
void Powersupply_B590::run_dialog_0x65(void)
{
    ...
    switch (state)
    {
    case NO_TASK:                emit info("Нет задачи");    break;
    case TASK_CALIBRATION_ADC_U: emit info("Запуск калибровки АЦП по напряжению");    break;
    case TASK_CALIBRATION_ADC_I: emit info("Запуск калибровки АЦП по току");    break;
    case TASK_CALIBRATION_DAC_U: emit info("Запуск калибровки ЦАП по напряжению");    break;
    case TASK_CALIBRATION_DAC_I: emit info("Запуск калибровки ЦАП по току");    break;
    case TASK_SLP:               emit info("Режим SLP");    break;
    case TASK_SHOW_ERROR:        emit info("Отображение ошибки");    break;
    case TASK_RETURN_MENU:       emit info("Выход в основное меню");    break;
...
void Powersupply_B590::run_dialog_0x66(void)
{
    ...
    QGridLayout *grid = new QGridLayout();
    grid->addWidget(new QLabel("U=0 I=1"),      0, 0);
    grid->addWidget(sb_state,                   0, 1);
    grid->addWidget(new QLabel("Index point"),  1, 0);
...
void Powersupply_B590::run_dialog_0x67(void)
{
    ...
    QGridLayout *grid = new QGridLayout();
    grid->addWidget(new QLabel("Speed cooler"), 0, 0);
...
void Powersupply_B590::run_dialog_0x6C(void)
{
    ...
    QGridLayout *grid = new QGridLayout();
    grid->addWidget(new QLabel("Number string"), 0, 0);
...
void Powersupply_B590::run_dialog_0x6D(void)
{
    ...
    QGridLayout *grid = new QGridLayout();
    grid->addWidget(new QLabel("ID"),   1, 0);
...
void Powersupply_B590::run_dialog_0x6E(void)
{
    ...
    QGridLayout *grid = new QGridLayout();
    grid->addWidget(new QLabel("Profile"),  0, 0);
    grid->addWidget(sb_profile,             0, 1);
    grid->addWidget(new QLabel("Repeat"),   1, 0);
...
{
    uint8_t state_ADC_U = 0;
    uint8_t state_ADC_I = 0;
    uint8_t state_ADC_Ui = 0;
...
void Powersupply_B590::run_dialog_0x72(void)
{
    ...
    QGridLayout *grid = new QGridLayout();
    grid->addWidget(new QLabel("conf_ADC"), 0, 0);
...
{
    uint8_t count_ReStart_ADC_1 = 0;
    uint8_t count_ReStart_ADC_0 = 0;
    uint8_t count_ReStart_ADC_2 = 0;
    uint16_t count_Error_Answer_ADC = 0;
...
//--------------------------------------------------------------------------------
#if 0
#define B590_CMD_0x54_TEXT "Б5-90 функция 0x54: Запрос данных по профилям.")
#define B590_CMD_0x56_TEXT "Б5-90 функция 0x56: Чтение профиля по точкам.")
#define B590_CMD_0x5E_TEXT "Б5-90 функция 0x5E: Запись данных точки в профиль для исполнения.")
#define B590_CMD_0x5F_TEXT "Б5-90 функция 0x5F: Запуск профиля на исполнение.")
#define B590_CMD_0x6E_TEXT "Б5-90 функция 0x6E: Запись количества повторов в профиль.")
#define B590_CMD_0x6F_TEXT "Б5-90 функция 0x6F: Очистка профиля")
#endif
//--------------------------------------------------------------------------------
...
int Powersupply_B590::print_last_error(void)
{
    switch(last_error)
    {
    case E_B590_NO_ERROR:                   emit info("NO ERROR"); break;
    case E_B590_ERROR_INDEX_NOT_CORRECT:    emit error("Индекс не корректен"); break;
    case E_B590_ERROR_PORT_NOT_OPEN:        emit error("Порт источника питания Б5-90 не открыт!"); break;
    case E_B590_ERROR_BREAK:                emit error("E_B590_ERROR_BREAK"); break;
    case E_B590_ERROR_MAX_REPEAT:           emit error("E_B590_ERROR_MAX_REPEAT"); break;
    case E_B590_ERROR_NO_ANSWER:            emit error(QString("%1 не отвечает").arg(POWER_SUPPLY_NAME)); break;
    case E_B590_ERROR_BIG_ANSWER:           emit error("Неверная длина пакета (слишком большой)"); break;
    case E_B590_ERROR_SMALL_ANSWER:         emit error("Неверная длина пакета (слишком маленький)"); break;
    case E_B590_ERROR_BAD_SERNO:            emit error("Неверный серийный номер"); break;
    case E_B590_ERROR_BAD_ADDRESS:          emit error("Неверный адрес"); break;
    case E_B590_ERROR_BAD_CRC:              emit error("Ошибка контрольной суммы"); break;
    case E_B590_ERROR_BAD_COMMAND:          emit error("Плохая команда"); break;
    case E_B590_ERROR_BAD_LEN:              emit error("Неверная длина"); break;
    case E_B590_ERROR_UNKNOWN_COMMAND:      emit error("Неизвестная команда"); break;
    case E_B590_INVALID_FLOAT:              emit error("E_B590_INVALID_FLOAT"); break;
    case E_B590_INVALID_LIMIT:              emit error("E_B590_INVALID_LIMIT"); break;
    case E_B590_INVALID_DELTA:              emit error("E_B590_INVALID_DELTA"); break;
    case E_B590_BREAK_MEASURING:            emit error("E_B590_BREAK_MEASURING"); break;
    case E_B590_UNKNOWN_PROFILE:            emit error("Недопустимый номер профиля"); break;
    case E_B590_UNKNOWN_POINT:              emit error("Недопустимый номер точки в профиле"); break;
    case E_B590_PROFILE_EMPTY:              emit error("Профиль пуст"); break;
    case E_B590_ERROR_SIGNATURE:            emit error("Не совпадает сигнатура"); break;
    case E_B590_NUMBER_PROFILE_ERROR:       emit error("Недопустимый номер профиля."); break;
    case E_B590_NUMBER_POINT_ERROR:         emit error("Недопустимый номер точки."); break;
    case E_B590_NUMBER_STRING_ERROR:        emit error("Недопустимый номер строки."); break;
    case E_B590_BUSY:                       emit error("Прибор занят выполнением команды"); break;
    case E_B590_NUMBER_REPEAT_ERROR:        emit error("Недопустимое количество повторов"); break;
    case E_B590_LO_VOLTAGE:                 emit error("Значение напряжение ниже минимально допустимого."); break;
    case E_B590_HI_VOLTAGE:                 emit error("Превышено максимальное значение напряжения."); break;
    case E_B590_LO_CURRENT:                 emit error("Значение тока ниже минимально допустимого."); break;
    case E_B590_HI_CURRENT:                 emit error("Превышено максимальное значение тока."); break;
    case E_B590_NO_REVERSE:                 emit error("Нет платы реверса - попытка использовать реверс тока."); break;
    case E_B590_MAX_POWER:                  emit error("Превышена максимальная мощность источника."); break;
    case E_B590_LO_TIME:                    emit error("Значение времени ниже минимально допустимого."); break;
    case E_B590_HI_TIME:                    emit error("Превышено максимальное значение времени."); break;
    case E_B590_ERROR_POINT:                emit error("Неверный порядок точек при записи."); break;
    case E_B590_PROFILE_IS_RUN:             emit error("Попытка записи в исполняемый профиль."); break;
    case E_B590_CALIBRATION_RUNNING:        emit error("Производится процесс калибровки."); break;
    case E_B590_NO_RC:                      emit error("Нет захвата управления по данному интерфейсу."); break;
    case E_B590_COMMAND_ON_CONSTRUCTION:    emit error("Пока не сделано"); break;
...
//--------------------------------------------------------------------------------
void Powersupply_B590::updateText(void)
{

}
//--------------------------------------------------------------------------------
bool Powersupply_B590::programm_is_exit(void)
{
    return true;
}
//--------------------------------------------------------------------------------
void Powersupply_B590::load_setting(void)
{

}
//--------------------------------------------------------------------------------
void Powersupply_B590::save_setting(void)
{
