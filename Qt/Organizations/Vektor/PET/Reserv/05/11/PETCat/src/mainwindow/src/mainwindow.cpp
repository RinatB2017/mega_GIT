...
void MainWindow::f_calibration(void)
{
    ...
    pet_config.host = QHostAddress::LocalHost;
    pet_config.addr = 0;
    pet_config.port = 666;
    Calibration *w = new Calibration(pet_config, this);
#endif
    if(w)
    {
...
void MainWindow::f_update_firmware(void)
{
    ...
    pet_config.addr = 0;
    pet_config.port = 666;
    UpdateFirmware *w = new UpdateFirmware(pet_config, this);
#endif
    if(w)
    {
...
void MainWindow::closeEvent(QCloseEvent *event)
{
    ...
    if(btn == MyWidget::Yes)
    {
        ...
        {
            bool ok = main_tab->widget(0)->close();
            if(ok)
            {
                qDebug() << "close" << main_tab->tabText(0);
