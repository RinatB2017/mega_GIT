...
QwtText QwtPlotPicker::trackerTextF( const QPointF &pos ) const
{
    ...
    switch ( rubberBand() )
    {
        ...
        case VLineRubberBand:
            ...
            break;
        default:
            //TODO отображение координат рядом с курсором
    ...
    switch ( selectionType )
    {
        ...
        case QwtPickerMachine::RectSelection:
        {
            if ( points.count() >= 2 )
            {
                ...
                Q_EMIT selected( transform( rect ) );
            }
            break;
        }
...
QRect QwtPlotPicker::transform( const QRectF &rect ) const
{
    ...
    return QwtScaleMap::invTransform( xMap, yMap, rect ).toRect();
}
...
/*!
    ...
    \sa transform()
*/
QPointF QwtPlotPicker::invTransform2( const QPoint &pos ) const
{
