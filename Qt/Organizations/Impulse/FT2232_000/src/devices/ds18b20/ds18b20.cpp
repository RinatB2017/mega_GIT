...
void DS18B20::test(void)
{
    ...
    for(int n=0; n<20; n++)
    {
        emit debug(QString("test %1").arg(n));
        ...
        if(ftStatus == FT_OK)
        {
            if(!dwNumInputBuffer)
            {
                emit error(QString("return %1 bytes").arg(dwNumInputBuffer));
            }
            else
            {
                ...
                ftStatus = ft2232h->read(&buf[0], dwNumInputBuffer, &dwNumBytesRead);
                if(ftStatus == FT_OK)
                {
                    emit info(QString("read %1 bytes [0x%2]")
                              .arg(dwNumBytesRead)
                              .arg((int)buf[0], 0, 16));
                }
            }
