...
void CreatorBox::init_widgets(void)
{
    ...
    //---
    connect(ui->btn_create_dat_file, SIGNAL(clicked(bool)), this, SLOT(create_dat3_file()));
}
//--------------------------------------------------------------------------------
...
void CreatorBox::create_dat3_file(void)
{
    ...
    for(int n=0; n<max_n; n++)
    {
        ...
        //---
        //TODO
        int rand_flag = rand() % 100;
        event.flags = 0;
        if(rand_flag < ui->sb_PET_EVENT_FLAG_FAULT->value())         event.flags |= PET_EVENT_FLAG_FAULT;
        if(rand_flag < ui->sb_PET_EVENT_FLAG_TS_INVALID->value())    event.flags |= PET_EVENT_FLAG_TS_INVALID;
        if(rand_flag < ui->sb_PET_EVENT_FLAG_GATE_OVERLAY->value())  event.flags |= PET_EVENT_FLAG_GATE_OVERLAY;
        if(rand_flag < ui->sb_PET_EVENT_FLAG_QDC_OVERFLOW->value())  event.flags |= PET_EVENT_FLAG_QDC_OVERFLOW;
        if(rand_flag < ui->sb_PET_EVENT_FLAG_QDC_UNDERFLOW->value()) event.flags |= PET_EVENT_FLAG_QDC_UNDERFLOW;
        if(rand_flag < ui->sb_PET_EVENT_FLAG_ADC_OVERFLOW->value())  event.flags |= PET_EVENT_FLAG_ADC_OVERFLOW;
        if(rand_flag < ui->sb_PET_EVENT_FLAG_ADC_UNDERFLOW->value()) event.flags |= PET_EVENT_FLAG_ADC_UNDERFLOW;
        if(rand_flag < ui->sb_PET_EVENT_FLAG_SYNC->value())          event.flags |= PET_EVENT_FLAG_SYNC;
