...
//#include "v764_1_serialport_test.hpp"
...
#else
    serial = new SerialPort_V764_2_test();
#endif
...
#endif
#ifdef ADD_TOOLBAR
            QToolBar *tb = multimeter->add_test_toolbar();
            Q_ASSERT(tb);
            tb->setObjectName("test_toolbar");

            mw->addToolBar(Qt::LeftToolBarArea, tb);
#endif
        }
    });
}
...
void Multimeter_V764_1::port_read(const QByteArray &data)
{
    ...
    emit debug(QString("Multimeter_V764_1 read: [%1]").arg(data.toHex().toUpper().data()));
    data_multimeter.append(data);
}
//--------------------------------------------------------------------------------
...
#endif
    {
        ...
        foreach (int speed, speeds)
        {
            ...
            if(ok)
            {
                ...
                {
                    emit info(QString("Пробуем скорость %1").arg(speed));
                    init_modbus();

                    int err;
                    err = send_cmd_0x46();
                    if(err == E_V764_1_NO_ERROR)
                    {
                        emit info(QString("Мультиметр В7-64/2 найден на скорости %1").arg(speed));
                        return true;
                    }
                    else
                    {
...
//--------------------------------------------------------------------------------
bool Multimeter_V764_1::find_address(int begin_address, int end_address, int *r_address)
{
    serial->setPortName("COM22");
    bool ok = serial->setBaudRate(9600);
    if(ok)
    {
        if(serial->serial_open())
        {
            for(int n=begin_address; n<=end_address; n++)
            {
                init_modbus();

                address = n;
                emit info(QString("Проверяем адрес %1").arg(address));
                int err = send_cmd_0x46();
                if(err == E_V764_1_NO_ERROR)
                {
                    emit info(QString("Мультиметр В7-64/2 найден на адресе %1").arg(address));
                    *r_address = address;
                    return true;
                }
                else
...
//--------------------------------------------------------------------------------
int Multimeter_V764_1::init_modbus(void)
{
    if(is_silence == false)
    {
...
//--------------------------------------------------------------------------------
int Multimeter_V764_1::init_terminal(void)
{
...
int Multimeter_V764_1::send_cmd_0x41(void)
{
    if(is_silence == false)
    {
        emit info(V764_1_CMD_0x41_TEXT);
    }
    if(serial->isOpen() == false)
    {
        last_error = E_V764_1_ERROR_PORT_NOT_OPEN;
    ...
    question.header.address = get_address();
    question.header.cmd = V764_1_CMD_0x41;
    question.header.count_data = 0;
    question.crc16 = CRC::powersupply_crc16((unsigned char *)&question,
                                            sizeof(question) - 2);
                                     ...
                                     unsigned char time)
{
    if(is_silence == false)
    {
        emit info(V764_1_CMD_0x42_TEXT);
    }
    if(serial->isOpen() == false)
    {
        last_error = E_V764_1_ERROR_PORT_NOT_OPEN;
    ...
    question.header.address = 0xFE;
    question.header.cmd = V764_1_CMD_0x42;
    question.header.count_data = 2;
    question.baud = baud;
    question.time = time;
    question.crc16 = CRC::powersupply_crc16((unsigned char *)&question,
                                            sizeof(question) - 2);
...
int Multimeter_V764_1::send_cmd_0x45(void)
{
    if(is_silence == false)
    {
        emit info(V764_1_CMD_0x45_TEXT);
    ...
    question.header.address = address;
    question.header.cmd = V764_1_CMD_0x45;
    question.header.count_data = 0;
    question.crc16 = CRC::powersupply_crc16((unsigned char *)&question,
                                            sizeof(question) - 2);
    ...
    int err = check_packet_answer(V764_1_CMD_0x45, sizeof(CMD_V7642_0x45_ANSWER));
    if(err != E_V764_1_NO_ERROR)
    {
...
int Multimeter_V764_1::send_cmd_0x46(void)
{
    ...
    question.header.address = address;
    question.header.cmd = V764_1_CMD_0x46;
    question.header.count_data = 0;
    question.crc16 = CRC::powersupply_crc16((unsigned char *)&question,
                                            sizeof(question) - 2);
    ...
    {
        serial->write(ba.mid(n, 1));
        emit trace(QString("%1").arg(ba.mid(n, 1).toHex().toUpper().data()));
    }
#else
    serial->write(ba);
#endif
    ...
    int err = check_packet_answer(V764_1_CMD_0x46, sizeof(CMD_V7642_0x46_ANSWER));
    if(err != E_V764_1_NO_ERROR)
    {
        return last_error;
    }
    ...
    if(is_silence == false)
    {
        ...
        emit debug(QString("address %1").arg(answer->header.address));
        emit debug(QString("cmd 0x%1").arg(answer->header.cmd, 0, 16));
        emit debug(QString("count_data %1").arg(answer->header.count_data));
        emit debug(QString("type %1").arg(answer->type));
        emit debug(QString("year %1").arg(answer->year));
        emit debug(QString("month %1").arg(answer->month));
        emit debug(QString("serno_lo %1").arg(answer->serno_lo));
        emit debug(QString("serno_hi %1").arg(answer->serno_hi));
        emit debug(QString("baud %1").arg(answer->baud));
        emit debug(QString("time_waiting %1").arg(answer->time_waiting));
        emit debug(QString("address_station %1").arg(answer->address_station));
        emit debug(QString("byte_test %1").arg(answer->byte_test));
    }
...
int Multimeter_V764_1::send_cmd_0x47(int *value_uV)
{
    if(is_silence == false)
    {
        ...
        emit debug(QString("delay_ms: %1").arg(delay_ms));
    }
    if(serial->isOpen() == false)
    {
        last_error = E_V764_1_ERROR_PORT_NOT_OPEN;
    ...
    question.header.address = address;
    question.header.cmd = V764_1_CMD_0x47;
    question.header.count_data = 0;
    question.crc16 = CRC::powersupply_crc16((unsigned char *)&question,
                                            sizeof(question) - 2);
    ...
    wait(delay_ms);
#else
    wait(sizeof(V764_1_CMD_0x47), 200, delay_ms);
#endif

    int err = check_packet_answer(V764_1_CMD_0x47, sizeof(CMD_V7642_0x47_ANSWER));
    if(err != E_V764_1_NO_ERROR)
    {
        return last_error;
    }
    ...
    if(answer->error & 0x01)
    {
        last_error = E_V764_1_OVERLOAD;
        return last_error;
    }
    if(answer->error & 0x02)
    {
        last_error = E_V764_1_LO_FREQ;
        return last_error;
    }

    int result = answer->value;
    if(answer->state & 0x80) result = -result;
...
int Multimeter_V764_1::send_cmd_0x48(unsigned char new_address)
{
    if(is_silence == false)
    {
        emit info(V764_1_CMD_0x48_TEXT);
        emit debug(QString("address %1").arg(address));
    }
    if(serial->isOpen() == false)
    {
    ...
    question.header.address = 0xFE; //address;
    question.header.cmd = V764_1_CMD_0x48;
    question.header.count_data = 1;
    question.data_0 = new_address;
    question.crc16 = CRC::powersupply_crc16((unsigned char *)&question,
                                            sizeof(question) - 2);
    ...
    wait(delay_ms);
#else
    wait(sizeof(V764_1_CMD_0x48), 200, delay_ms);
#endif

    int err = check_packet_answer(V764_1_CMD_0x48, sizeof(CMD_V7642_0x48_ANSWER));
    if(err != E_V764_1_NO_ERROR)
    {
        return last_error;
    }
...
int Multimeter_V764_1::send_cmd_0x53(unsigned char number_button_1_16)
{
    ...
    question.header.address = address;
    question.header.cmd = V764_1_CMD_0x53;
    question.header.count_data = sizeof(question);
    switch(number_button_1_16)
    {
    case BUTTON_NUMBER_UDC: question.data_0 = BUTTON_CODE_UDC;    break;
    case BUTTON_NUMBER_IDC: question.data_0 = BUTTON_CODE_IDC;    break;
    case BUTTON_NUMBER_R:   question.data_0 = BUTTON_CODE_R;      break;
    case BUTTON_NUMBER_UAC: question.data_0 = BUTTON_CODE_UAC;    break;
    case BUTTON_NUMBER_IAC: question.data_0 = BUTTON_CODE_IAC;    break;
    case BUTTON_NUMBER_HZ:  question.data_0 = BUTTON_CODE_HZ;     break;
    case BUTTON_NUMBER_MHZ: question.data_0 = BUTTON_CODE_MHZ;    break;
    case BUTTON_NUMBER_AVR: question.data_0 = BUTTON_CODE_AVR;    break;
    }
    ...
    int err = check_packet_answer(V764_1_CMD_0x53, sizeof(CMD_V7642_0x53_ANSWER));
    if(err != E_V764_1_NO_ERROR)
    {
        return print_last_error();
    }
    ...
    if(acmd.bites.bit7 == 0)
    {
        ...
        {
            last_error = E_V764_1_ERROR_BIG_ANSWER;
            return last_error;
        }
        if((unsigned int)data_multimeter.length() < sizeof(CMD_V7642_0x53_ANSWER))
        {
            last_error = E_V764_1_ERROR_SMALL_ANSWER;
            return last_error;
        }
    }
    else
    {
        ...
        emit error(QString("error %1").arg(err->error));
    }
...
//--------------------------------------------------------------------------------
void Multimeter_V764_1::wait(int time_msec)
{
#ifdef TEST
    Q_UNUSED(time_msec)
#else
...
//--------------------------------------------------------------------------------
void Multimeter_V764_1::wait(int max_len,
                             int min_time_msec,
                             int max_time_msec)
{
#ifdef TEST
...
//--------------------------------------------------------------------------------
void Multimeter_V764_1::check_multimeter_v762_2(void)
{
...
//--------------------------------------------------------------------------------
void Multimeter_V764_1::run_dialog_0x41(void)
{
...
//--------------------------------------------------------------------------------
void Multimeter_V764_1::run_dialog_0x42(void)
{
    QDialog *dialog = new QDialog();
    dialog->setWindowTitle(V764_1_CMD_0x42_TEXT);
...
//--------------------------------------------------------------------------------
void Multimeter_V764_1::run_dialog_0x45(void)
{
...
//--------------------------------------------------------------------------------
void Multimeter_V764_1::run_dialog_0x46(void)
{
    int err = send_cmd_0x46();
    if(err != E_V764_1_NO_ERROR)
    {
...
//--------------------------------------------------------------------------------
void Multimeter_V764_1::run_dialog_0x47(void)
{
    int value_mkV = 0;
...
//--------------------------------------------------------------------------------
void Multimeter_V764_1::run_dialog_0x48(void)
{
    QDialog *dialog = new QDialog();
    dialog->setWindowTitle(V764_1_CMD_0x48_TEXT);
...
//--------------------------------------------------------------------------------
void Multimeter_V764_1::run_dialog_0x53(void)
{
    QDialog *dialog = new QDialog();
    dialog->setWindowTitle(V764_1_CMD_0x53_TEXT);
...
//--------------------------------------------------------------------------------
QMenu *Multimeter_V764_1::add_menu(void)
{
    ...
    action_m_0x41->setText(V764_1_CMD_0x41_TEXT);
    action_m_0x42->setText(V764_1_CMD_0x42_TEXT);
    action_m_0x45->setText(V764_1_CMD_0x45_TEXT);
    action_m_0x46->setText(V764_1_CMD_0x46_TEXT);
    action_m_0x47->setText(V764_1_CMD_0x47_TEXT);
    action_m_0x48->setText(V764_1_CMD_0x48_TEXT);
    action_m_0x53->setText(V764_1_CMD_0x53_TEXT);
    ...
    connect(action_m_0x41, &QAction::triggered, this, &Multimeter_V764_1::run_dialog_0x41);
    connect(action_m_0x42, &QAction::triggered, this, &Multimeter_V764_1::run_dialog_0x42);
    connect(action_m_0x45, &QAction::triggered, this, &Multimeter_V764_1::run_dialog_0x45);
    connect(action_m_0x46, &QAction::triggered, this, &Multimeter_V764_1::run_dialog_0x46);
    connect(action_m_0x47, &QAction::triggered, this, &Multimeter_V764_1::run_dialog_0x47);
    connect(action_m_0x48, &QAction::triggered, this, &Multimeter_V764_1::run_dialog_0x48);
    connect(action_m_0x53, &QAction::triggered, this, &Multimeter_V764_1::run_dialog_0x53);
...
int Multimeter_V764_1::print_last_error(void)
{
    ...
    case E_V764_1_NO_ERROR:
        break;
    case E_V764_1_ERROR_INDEX_NOT_CORRECT:  emit error("E_V764_1_ERROR_INDEX_NOT_CORRECT");     break;
    case E_V764_1_ERROR_PORT_NOT_OPEN:      emit error("Порт мультиметра В7-64/2 не открыт!");  break;
    case E_V764_1_ERROR_BREAK:              emit error("E_V764_1_ERROR_BREAK");                 break;
    case E_V764_1_ERROR_MAX_REPEAT:         emit error("E_V764_1_ERROR_MAX_REPEAT");            break;
    case E_V764_1_ERROR_NO_ANSWER:          emit error("Мультиметр не отвечает");               break;
    case E_V764_1_ERROR_BIG_ANSWER:         emit error("Неверная длина пакета (слишком большая)");      break;
    case E_V764_1_ERROR_SMALL_ANSWER:       emit error("Неверная длина пакета (слишком маленькая)");    break;
    case E_V764_1_ERROR_BAD_SERNO:          emit error("Неверный серийный номер");              break;
    case E_V764_1_ERROR_BAD_ADDRESS:        emit error("Неверный адрес");                       break;
    case E_V764_1_ERROR_BAD_CRC:            emit error("Неверная контрольная сумма");           break;
    case E_V764_1_ERROR_BAD_COMMAND:        emit error("Неверная команда");                     break;
    case E_V764_1_ERROR_BAD_LEN:            emit error("Неверная длина");                       break;
    case E_V764_1_ERROR_UNKNOWN_COMMAND:    emit error("Неизвестная команда");                  break;
    case E_V764_1_ERROR_SIZE_NOT_CORRECT:   emit error("Длина данных не соответствует завленной");  break;
    case E_V764_1_UNKNOWN_ERROR:            emit error("Неизвестная ошибка");                   break;
    case E_V764_1_INVALID_FLOAT:            emit error("E_V764_1_INVALID_FLOAT");               break;
    case E_V764_1_INVALID_LIMIT:            emit error("E_V764_1_INVALID_LIMIT");               break;
    case E_V764_1_INVALID_DELTA:            emit error("E_V764_1_INVALID_DELTA");               break;
    case E_V764_1_BREAK_MEASURING:          emit error("E_V764_1_BREAK_MEASURING");             break;
    case E_V764_1_ILLEGAL_FUNCTION:         emit error("Команда не реализована (недопустимый номер функции");   break;
    case E_V764_1_ILLEGAL_DATA_VALUE:       emit error("Не корректные данные или не верный CRC");   break;
    case E_V764_1_OVERLOAD:                 emit error("Перегрузка");                           break;
    case E_V764_1_LO_FREQ:                  emit error("Низкая частота");                       break;
...
#endif
//--------------------------------------------------------------------------------
int Multimeter_V764_1::check_packet_answer(unsigned char cmd, int packet_len)
{
    if(data_multimeter.isEmpty())
    {
        last_error = E_V764_1_ERROR_NO_ANSWER;
        return last_error;
    }

    CMD_V7642_ERROR_ANSWER *answer = (CMD_V7642_ERROR_ANSWER *)data_multimeter.constData();
    unsigned char answer_cmd = answer->header.cmd & 0x7F;
    bool flag_error = answer->header.cmd & 0x80;

    if(answer_cmd != cmd)
    {
        last_error = E_V764_1_ERROR_BAD_COMMAND;
        return last_error;
    }
    if(flag_error)
    {
        ...
        {
        case 1:
            last_error = E_V764_1_ILLEGAL_FUNCTION;
            return last_error;
    ...
    if(header->count_data != len)
    {
        ...
        emit debug(QString(tr("check_packet_answer: size (пришло) %1 < (ожидалось) %2"))
                   ...
                   .arg(len));
        last_error = E_V764_1_ERROR_SIZE_NOT_CORRECT;
        return last_error;
    }
    last_error = E_V764_1_NO_ERROR;
    return last_error;
