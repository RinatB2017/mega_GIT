...
void Calibration::init(void)
{
    ...
    PET_RESULT err = comm->set_comm_param(addr, PETCat_options::get_port_cmd());
    if(err != PET_SUCCESS)
    {
    ...
    //---
    err = pet_brg->set_comm_param(p_pet_config.at(0).host,
...
void Calibration::run_calib_coords(int num_channel)
{
    ...
    for(int n=0; n<max_cnt_calib; n++)
    {
        ...
        if((k != 1.0) || (b != 0))
        {
            ...
            if(ok == false)
            {
                emit error("Калибровка канала завершена с ошибкой");
                return;
            }
        }
        else
