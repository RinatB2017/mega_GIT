...
void MKT_box::init(void)
{
    ...
    //---
    connect(ui->btn_start,  SIGNAL(clicked(bool)),  this,   SLOT(f_start()));
    connect(ui->btn_stop,   SIGNAL(clicked(bool)),  this,   SLOT(f_stop()));
    connect(ui->btn_reset,  SIGNAL(clicked(bool)),  this,   SLOT(f_reset()));
    connect(ui->btn_refresh,    SIGNAL(clicked(bool)),  this,   SLOT(f_refresh_ports()));
    connect(ui->btn_set_min_max,    SIGNAL(clicked(bool)),  this,   SLOT(f_set_min_max()));
    //---
...
bool MKT_box::get_ver_firmvare(void)
{
    ...
    int hi = value / 100;
    int lo = value - (hi * 100);
    emit message(QString("value %1").arg(value));
