...
bool PET_comm::bind(QHostAddress address, quint16 port)
{
    ...
    if(!ok)
    {
        last_error = QString("PET_comm not binding: %1").arg(socket->errorString());
        return false;
    }
    return true;
}
...
{
    if(host.isNull())       return PET_ERR_BAD_ADDR;
    if(port<MIN_UDP_PORT)   return PET_ERR_BAD_ADDR;
    if(port>=MAX_UDP_PORT)  return PET_ERR_BAD_ADDR;
                                  ...
                                  QByteArray r_data)
{
    ...
    //if(i_addr != frame->addr) return;
    //if(i_cmd != frame->cmd) return;
