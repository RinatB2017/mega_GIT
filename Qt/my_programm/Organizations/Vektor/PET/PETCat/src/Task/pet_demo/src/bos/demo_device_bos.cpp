...
#include "pet_errno.h"
//--------------------------------------------------------------------------------
Device_BOS::Device_BOS(QObject *parent) : Device_base(parent)
{
    init();
}
//--------------------------------------------------------------------------------
void Device_BOS::init(void)
{
    connect_log();
    ...
    bos_func_list.append(&Device_BOS::f_PET_CMD_SET_CONTROL);
    bos_func_list.append(&Device_BOS::f_PET_CMD_GET_CONTROL);
    bos_func_list.append(&Device_BOS::f_PET_CMD_SET_CH_CONTROL);
    bos_func_list.append(&Device_BOS::f_PET_CMD_GET_CH_CONTROL);
    bos_func_list.append(&Device_BOS::f_PET_CMD_GET_STATE);
    bos_func_list.append(&Device_BOS::f_PET_CMD_GET_CH_STATE);
    bos_func_list.append(&Device_BOS::f_PET_CMD_UPDATE);
    ...
    control.ip_address[0] = 6;
    control.ip_address[1] = 0;
    control.ip_address[2] = 0;
    control.ip_address[3] = 0;
    ...
    control.mac_addr[0] = 0;
    control.mac_addr[1] = 0;
    control.mac_addr[2] = 0;
    control.mac_addr[3] = 0;
    control.mac_addr[4] = 0;
    control.mac_addr[5] = 0;

    control.cmd_port = PET_NET_CMD_PORT_DEFAULT;
    control.event_port = PET_NET_EVENT_PORT_DEFAULT;

    control.event_addr[0] = 0;
    control.event_addr[1] = 0;
    control.event_addr[2] = 0;
    control.event_addr[3] = 0;
    //---
    //state.state = 0xFF;
    //---
    ch_state.channel = 0;
...
//--------------------------------------------------------------------------------
void Device_BOS::connect_log(void)
{
#ifdef NO_LOG
...
//--------------------------------------------------------------------------------
PET_RESULT Device_BOS::check(QByteArray input_packet, QByteArray *output_packet)
{
    foreach (base_func base_f, base_func_list)
    {
        PET_RESULT res = PET_ERR_NONE;
        res = (this->*base_f)(input_packet, output_packet);
        if(res == PET_SUCCESS)
        {
    ...
    foreach (bos_func bos_f, bos_func_list)
    {
        PET_RESULT res = PET_ERR_NONE;
        res = (this->*bos_f)(input_packet, output_packet);
        if(res == PET_SUCCESS)
        {
            //emit debug(input_packet.toHex().toUpper());
            return PET_SUCCESS;
...
//--------------------------------------------------------------------------------
PET_RESULT Device_BOS::f_PET_CMD_SET_CONTROL(QByteArray input_packet,
                                             QByteArray *output_packet)
{
    ...
    if(i_frame->len != sizeof(pet_bos_ctrl))
    {
        emit error(QString("i_frame->len = %1").arg(i_frame->len));
        emit error(QString("sizeof(pet_bos_ctrl) = %1").arg(sizeof(pet_bos_ctrl)));
        emit error("i_frame->len != sizeof(pet_bos_ctrl)");
...
//--------------------------------------------------------------------------------
PET_RESULT Device_BOS::f_PET_CMD_GET_CONTROL(QByteArray input_packet,
                                             QByteArray *output_packet)
{
    Q_CHECK_PTR(output_packet);
    if(output_packet == nullptr)
...
//--------------------------------------------------------------------------------
PET_RESULT Device_BOS::f_PET_CMD_GET_STATE(QByteArray input_packet,
                                           QByteArray *output_packet)
{
    Q_CHECK_PTR(output_packet);
    if(output_packet == nullptr)
...
//--------------------------------------------------------------------------------
PET_RESULT Device_BOS::f_PET_CMD_GET_CH_STATE(QByteArray input_packet,
                                              QByteArray *output_packet)
{
    Q_CHECK_PTR(output_packet);
    if(output_packet == nullptr)
...
//--------------------------------------------------------------------------------
PET_RESULT Device_BOS::f_PET_CMD_SET_CH_CONTROL(QByteArray input_packet,
                                                QByteArray *output_packet)
{
    Q_CHECK_PTR(output_packet);
    if(output_packet == nullptr)
...
//--------------------------------------------------------------------------------
PET_RESULT Device_BOS::f_PET_CMD_GET_CH_CONTROL(QByteArray input_packet,
                                                QByteArray *output_packet)
{
    Q_CHECK_PTR(output_packet);
    if(output_packet == nullptr)
...
//--------------------------------------------------------------------------------
PET_RESULT Device_BOS::f_PET_CMD_UPDATE(QByteArray input_packet,
                                        QByteArray *output_packet)
{
    Q_CHECK_PTR(output_packet);
    if(output_packet == nullptr)
    ...
    {
        emit error(QString("i_frame->len = %1").arg(i_frame->len));
        emit error(QString("(int)sizeof(pet_update_data) = %1").arg((int)sizeof(pet_update_data)));
        emit error("i_frame->len != (int)sizeof(pet_update_data)");
...
//--------------------------------------------------------------------------------
void Device_BOS::set_net_address(QHostAddress value)
{
    if(value.isNull())
    {
        return;
...
//--------------------------------------------------------------------------------
QHostAddress Device_BOS::get_net_address(void)
{
    UINT32_BYTES temp;
...
//--------------------------------------------------------------------------------
void Device_BOS::set_event_address(QHostAddress value)
{
    if(value.isNull())
    {
        return;
...
//--------------------------------------------------------------------------------
QHostAddress Device_BOS::get_event_address(void)
{
    UINT32_BYTES temp;
...
//--------------------------------------------------------------------------------
void Device_BOS::set_net_mask(quint32 value)
{
    UINT32_BYTES temp;
    temp.value = value;
...
//--------------------------------------------------------------------------------
void Device_BOS::set_net_gateway(quint32 value)
{
    UINT32_BYTES temp;
    temp.value = value;
...
//--------------------------------------------------------------------------------
void Device_BOS::set_mac_address(uint8_t mac_0,
                                 uint8_t mac_1,
                                 uint8_t mac_2,
                                 uint8_t mac_3,
                                 uint8_t mac_4,
...
//--------------------------------------------------------------------------------
QString Device_BOS::get_str_mac_address(void)
{
...
//--------------------------------------------------------------------------------
void Device_BOS::set_cmd_port(quint16 value)
{
    cmd_port = value;
}
//--------------------------------------------------------------------------------
quint16 Device_BOS::get_cmd_port(void)
{
    return cmd_port;
}
//--------------------------------------------------------------------------------
void Device_BOS::set_event_port(quint16 value)
{
    event_port = value;
}
//--------------------------------------------------------------------------------
quint16 Device_BOS::get_event_port(void)
{
    return event_port;
}
//--------------------------------------------------------------------------------
void Device_BOS::set_pet_address(uint8_t value)
{
    pet_address = value;
}
//--------------------------------------------------------------------------------
uint8_t Device_BOS::get_pet_address(void)
{
    return pet_address;
}
//--------------------------------------------------------------------------------
