...
#include <QtWidgets>
//--------------------------------------------------------------------------------
#include "pet_brg_sim_debug_page.hpp"
#include "ui_pet_brg_sim_debug_page.h"
#include "defines.hpp"
//--------------------------------------------------------------------------------
...
#include "pet_errno.h"
//--------------------------------------------------------------------------------
PET_brg_sim_debug_page::PET_brg_sim_debug_page(QWidget *parent) :
    PET_device_sim(parent),
    ui(new Ui::PET_brg_sim_debug_page)
{
    init();
}
//--------------------------------------------------------------------------------
PET_brg_sim_debug_page::~PET_brg_sim_debug_page()
{
    delete ui;
}
//--------------------------------------------------------------------------------
void PET_brg_sim_debug_page::init(void)
{
    ui->setupUi(this);
    ...
    connect(ui->btn_save_to_EEPROM, SIGNAL(clicked(bool)),  this,   SLOT(btn_save_to_EEPROM_click()));
    connect(ui->btn_set_default,    SIGNAL(clicked(bool)),  this,   SLOT(btn_set_default_click()));
}
//--------------------------------------------------------------------------------
void PET_brg_sim_debug_page::btn_save_to_EEPROM_click(void)
{
    save_to_EEPROM();
}
//--------------------------------------------------------------------------------
void PET_brg_sim_debug_page::btn_set_default_click(void)
{
    set_default();
}
//--------------------------------------------------------------------------------
PET_RESULT PET_brg_sim_debug_page::set(pet_brg_ctrl_t p_data)
{
...
//--------------------------------------------------------------------------------
PET_RESULT PET_brg_sim_debug_page::get(pet_brg_ctrl_t *p_data)
{
    Q_CHECK_PTR(p_data);
    //---
    bool ok = false;
...
//--------------------------------------------------------------------------------
void PET_brg_sim_debug_page::block_ctrl_interface(bool state)
{
...
//--------------------------------------------------------------------------------
void PET_brg_sim_debug_page::block_state_interface(bool state)
{
    Q_UNUSED(state)
}
//--------------------------------------------------------------------------------
