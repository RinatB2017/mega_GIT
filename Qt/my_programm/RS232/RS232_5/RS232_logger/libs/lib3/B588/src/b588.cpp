...
//--------------------------------------------------------------------------------
int B588::set_UI_parrot(int U, int I)
{
    bool ok = false;
...
//--------------------------------------------------------------------------------
int B588::set_vent_speed_0(void)
{
    bool ok = false;
...
//--------------------------------------------------------------------------------
int B588::set_vent_speed_max(void)
{
    bool ok = false;
...
//--------------------------------------------------------------------------------
int B588::set_vent_speed_auto(void)
{
    bool ok = false;
...
int B588::test_U(void)
{
    ...
    emit info(QString("MAGIC_B588_U = %1").arg(MAGIC_B588_U));
    ...
    for(new_setting_U = static_cast<uint16_t>(sb_begin->value()); new_setting_U < static_cast<unsigned long>(sb_end->value()); new_setting_U++)
    {
        pd->setValue(static_cast<uint16_t>(new_setting_U));
        if(pd->wasCanceled())
        {
            break;
        }
        ...
        V1 = powersupply->get_Correct_value_U(static_cast<uint16_t>(new_setting_U));
...
int B588::test_I(void)
{
    ...
    emit info("draw graphes");
    emit info(QString("MAGIC_B588_I = %1").arg(MAGIC_B588_I));
    unsigned long new_setting_I = 0;
    signed long V1 = 0;
    QProgressDialog *pd = new QProgressDialog("Operation in progress.", "Cancel", sb_begin->value(), sb_end->value(), this);
    pd->setWindowModality(Qt::WindowModal);
    for(new_setting_I = static_cast<uint16_t>(sb_begin->value()); new_setting_I < static_cast<unsigned long>(sb_end->value()); new_setting_I++)
    {
        pd->setValue(static_cast<uint16_t>(new_setting_I));
        if(pd->wasCanceled())
        {
            break;
        }
...
//--------------------------------------------------------------------------------
void B588::read_calibration_point_B588(void)
{
    bool ok = false;
    ...
    {
        ok = powersupply->get_calibration_point_I(static_cast<uint8_t>(n), &value);
        if(ok)
        {
            emit info(QString("dac_I[%1]=%2;")
                      .arg(n)
                      .arg(value));
            grapher->add_curve_data(static_cast<int>(curve_I), value);
        }
        else
        {
            powersupply->print_last_error();
...
void B588::createWidgets(void)
{
    ...
    ui->sb_max_current->setMinimum(MIN_CURRENT_B588);
    ui->sb_max_current->setMaximum(MAX_CURRENT_B588);

    ui->sb_first_U->setMinimum(MIN_VOLTAGE_B588);
    ui->sb_first_U->setMaximum(MAX_VOLTAGE_B588);

    ui->sb_first_I->setMinimum(MIN_CURRENT_B588);
    ui->sb_first_I->setMaximum(MAX_CURRENT_B588);

    ui->sb_second_U->setMinimum(MIN_VOLTAGE_B588);
    ui->sb_second_U->setMaximum(MAX_VOLTAGE_B588);

    ui->sb_second_I->setMinimum(MIN_CURRENT_B588);
    ui->sb_second_I->setMaximum(MAX_CURRENT_B588);
    ...
    //connect(ui->knob_U, SIGNAL(sliderReleased()), this, SLOT(set_U()));
    //connect(ui->knob_I, SIGNAL(sliderReleased()), this, SLOT(set_I()));
    ...
    connect(ui->btn_corridor, SIGNAL(toggled(bool)), this, SLOT(corridor(bool)));

    connect(ui->cb_ignore_bad_cmd, SIGNAL(toggled(bool)), this, SLOT(ignore_bad_cmd()));
...
{
    ui->sb_manual_U->setValue(static_cast<int>(value * 1000.0 + 0.5));
}
//--------------------------------------------------------------------------------
void B588::set_manual_I(double value)
{
    ui->sb_manual_I->setValue(static_cast<int>(value * 1000.0 + 0.5));
}
//--------------------------------------------------------------------------------
void B588::set_manual_U(int value)
{
    ui->knob_U->setValue(value / 1000.0);
}
//--------------------------------------------------------------------------------
void B588::set_manual_I(int value)
{
    ui->knob_I->setValue(value / 1000.0);
}
//--------------------------------------------------------------------------------
void B588::createTimer(void)
{
...
void B588::createPowersupply(void)
{
    ...
    connect(ui->btn_close_connect, &QPushButton::clicked, this, &B588::close_power_supply);
    ...
    {
        mw->addToolBarBreak();
        B588_toolbar *psBar_b588 = new B588_toolbar(tr("powersupply B5-88"), this);
...
//--------------------------------------------------------------------------------
void B588::createGrapherBox(void)
{
    ...
    curve_U = static_cast<uint>(grapher->add_curve("U", symbol_U));
    curve_I = static_cast<uint>(grapher->add_curve("I", symbol_I));
    curve_R = static_cast<uint>(grapher->add_curve("R", symbol_R));
    curve_W = static_cast<uint>(grapher->add_curve("W", symbol_W));
    curve_T = static_cast<uint>(grapher->add_curve("T", symbol_T));
...
//--------------------------------------------------------------------------------
void B588::timer_start(void)
{
    timer->start(ui->sb_interval->value());
}
//--------------------------------------------------------------------------------
void B588::timer_stop(void)
{
    timer->stop();
}
//--------------------------------------------------------------------------------
void B588::change_type_powersupply(int)
{
    //emit info("B588");
    ui->knob_U->setUpperBound(MAX_VOLTAGE_B588 / 1000);
    ui->knob_I->setUpperBound(MAX_CURRENT_B588 / 1000);

    ui->sb_manual_U->setMaximum(MAX_VOLTAGE_B588);
    ui->sb_manual_I->setMaximum(MAX_CURRENT_B588);
}
//--------------------------------------------------------------------------------
void B588::change_state_powersupply(int)
{
    ...
    case IN_PARROT:
        ui->knob_U->setUpperBound(MAX_VOLTAGE_B588);
        ui->knob_I->setUpperBound(MAX_CURRENT_B588);

        ui->sb_manual_U->setMaximum(0xFFFF);
        ui->sb_manual_I->setMaximum(0xFFFF);

        ui->sb_max_current->setMaximum(0xFFFF);
        ui->sb_first_U->setMaximum(0xFFFF);
        ui->sb_first_I->setMaximum(0xFFFF);
        ui->sb_second_U->setMaximum(0xFFFF);
        ui->sb_second_I->setMaximum(0xFFFF);
        break;
    }
}
//--------------------------------------------------------------------------------
void B588::set_UI(void)
{
    ...
    case IN_PARROT:
        ...
        break;
    case IN_NORMAL:
        ok = powersupply->set_UI(static_cast<uint16_t>(ui->knob_U->value() * 1000.0),
...
//--------------------------------------------------------------------------------
void B588::find_max_power(void)
{
    unsigned short value = MIN_VOLTAGE_B588;
    unsigned short increment = (MAX_VOLTAGE_B588 / 2);
...
void B588::extreme(void)
{
    ...
    {
        QCoreApplication::processEvents();
        ok = powersupply->set_UI(static_cast<uint16_t>(ui->sb_first_U->value()),
        ...
        if(ui->sb_delay_msec->value()>1) Sleeper::msleep(static_cast<ulong>(ui->sb_delay_msec->value()));
        get_UI();
        ok = powersupply->set_UI(static_cast<uint16_t>(ui->sb_second_U->value()),
...
//--------------------------------------------------------------------------------
void B588::find_max_power_start(void)
{
...
//--------------------------------------------------------------------------------
void B588::find_max_power_stop(void)
{
    flag_find_stop = true;
}
//--------------------------------------------------------------------------------
void B588::test_start(void)
{
    emit info("Test begin");
    extreme();
    emit info("Test end");
...
void B588::block_interface(bool state)
{
    ...
    ui->btn_corridor->setDisabled(state);
...
//--------------------------------------------------------------------------------
void B588::close_power_supply(void)
{
...
//--------------------------------------------------------------------------------
void B588::corridor(bool state)
{
    int err = -1;
    if(state)
    {
        ui->btn_corridor->setText("ON");
        err = corridor_on();
        if(err != E_B588_NO_ERROR) ui->btn_corridor->setChecked(false);
    }
    else
    {
        ui->btn_corridor->setText("OFF");
        err = corridor_off();
        if(err != E_B588_NO_ERROR) ui->btn_corridor->setChecked(false);
    }
}
//--------------------------------------------------------------------------------
int B588::corridor_on(void)
{
    bool ok = false;

    powersupply->set_address(ui->sb_address->value());
    ok = powersupply->corridor_on();
    if(!ok)
    {
...
void B588::get_UI(void)
{
    ...
    qreal u = static_cast<double>(out_voltage) / 1000.0;
    qreal i = static_cast<double>(out_current) / 1000.0;
    ...
    grapher->add_curve_data(static_cast<int>(curve_U), out_voltage);
    grapher->add_curve_data(static_cast<int>(curve_I), out_current);
    grapher->add_curve_data(static_cast<int>(curve_R), r);
    grapher->add_curve_data(static_cast<int>(curve_W), w);
    grapher->add_curve_data(static_cast<int>(curve_T), temp);
}
//--------------------------------------------------------------------------------
void B588::ignore_bad_cmd(void)
{
    emit info("B588");
    powersupply->set_ignore_bad_cmd(ui->cb_ignore_bad_cmd->isChecked());
}
...
//--------------------------------------------------------------------------------
bool B588::programm_is_exit(void)
{
    return true;
}
//--------------------------------------------------------------------------------
void B588::load_setting(void)
{

}
//--------------------------------------------------------------------------------
void B588::save_setting(void)
{
