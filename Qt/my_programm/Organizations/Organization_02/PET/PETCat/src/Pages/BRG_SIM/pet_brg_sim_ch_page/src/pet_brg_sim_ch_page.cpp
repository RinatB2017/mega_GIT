...
#include "pet_errno.h"
//--------------------------------------------------------------------------------
PET_brg_sim_ch_page::PET_brg_sim_ch_page(uint16_t channel, QWidget *parent) :
    MyWidget(parent),
    ui(new Ui::PET_brg_sim_ch_page),
    channel(channel)
{
    init();
}
//--------------------------------------------------------------------------------
PET_brg_sim_ch_page::~PET_brg_sim_ch_page()
{
    delete ui;
}
//--------------------------------------------------------------------------------
void PET_brg_sim_ch_page::init(void)
{
    ui->setupUi(this);
...
//--------------------------------------------------------------------------------
PET_RESULT PET_brg_sim_ch_page::set_ctrl(pet_brg_ch_ctrl_t p_data)
{
    ...
    if(i_mode != -1)        ui->cb_mode->setCurrentIndex(i_mode);                       else emit error(QString("i_mode %1").arg(p_data.mode));
    if(i_in_polarity != -1) ui->cb_in_polarity->setCurrentIndex(i_in_polarity);         else emit error(QString("i_in_polarity %1").arg(p_data.in_polarity));
    if(i_in_scale != -1)    ui->cb_in_scale->setCurrentIndex(i_in_scale);               else emit error(QString("i_in_scale %1").arg(p_data.in_scale));
    if(i_fir_mode != -1)    ui->cb_fir_mode->setCurrentIndex(i_fir_mode);               else emit error(QString("i_fir_mode %1").arg(p_data.fir_mode));
    if(i_fir_fc != -1)      ui->cb_fir_fc->setCurrentIndex(i_fir_fc);                   else emit error(QString("i_fir_fc %1").arg(p_data.fir_fc));
    if(i_cfd_mode != -1)    ui->cb_cfd_mode->setCurrentIndex(i_cfd_mode);               else emit error(QString("i_cfd_mode %1").arg(p_data.cfd_mode));
    if(i_cfd_lpf_mode != -1)    ui->cb_cfd_lpf_mode->setCurrentIndex(i_cfd_lpf_mode);   else emit error(QString("i_cfd_lpf_mode %1").arg(p_data.cfd_lpf_mode));
    if(i_cfd_fract != -1)   ui->cb_cfd_fract->setCurrentIndex(i_cfd_fract);             else emit error(QString("i_cfd_fract %1").arg(p_data.cfd_fract));
    if(i_tdc_mode != -1)    ui->cb_tdc_mode->setCurrentIndex(i_tdc_mode);               else emit error(QString("i_tdc_mode %1").arg(p_data.tdc_mode));
    if(i_blr_mode != -1)    ui->cb_blr_mode->setCurrentIndex(i_blr_mode);               else emit error(QString("i_blr_mode %1").arg(p_data.blr_mode));
    if(i_blr_fc != -1)      ui->cb_blr_fc->setCurrentIndex(i_blr_fc);                   else emit error(QString("i_blr_fc %1").arg(p_data.blr_fc));
    if(i_qdc_mode != -1)    ui->cb_qdc_mode->setCurrentIndex(i_qdc_mode);               else emit error(QString("i_qdc_mode %1").arg(p_data.qdc_mode));
...
//--------------------------------------------------------------------------------
PET_RESULT PET_brg_sim_ch_page::set_state(pet_brg_ch_state_t p_data)
{
    switch(p_data.state)
    {
...
//--------------------------------------------------------------------------------
PET_RESULT PET_brg_sim_ch_page::get_ctrl(pet_brg_ch_ctrl_t *p_data)
{
    Q_CHECK_PTR(p_data);
    //---
    bool ok = false;
...
//--------------------------------------------------------------------------------
void PET_brg_sim_ch_page::block_ctrl_interface(bool state)
{
...
//--------------------------------------------------------------------------------
void PET_brg_sim_ch_page::block_state_interface(bool state)
{
