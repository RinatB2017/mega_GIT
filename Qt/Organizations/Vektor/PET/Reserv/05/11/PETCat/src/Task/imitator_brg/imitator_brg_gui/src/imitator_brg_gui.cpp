...
void Imitator_BRG_GUI::init(void)
{
    ...
    connect(brg,    SIGNAL(message(QString)),   this,   SIGNAL(message(QString)));
    //---
    ...
    connect(ui->btn_refresh_state,  SIGNAL(clicked(bool)),  this,   SLOT(refresh_state()));
    //---
    refresh();
    //---
}
...
//--------------------------------------------------------------------------------
void Imitator_BRG_GUI::refresh(void)
{
    block_interface(true);
    bool ok = brg->dev_open();
    if(ok)
    {
        brg->dev_close();
