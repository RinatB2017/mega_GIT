...
void MainBox::init()
{
    ...
    ui->web_layout->addWidget(webview);

    ListWidget_Site *sites = new ListWidget_Site(webview, this);
    connect(sites, SIGNAL(info(QString)),  this, SIGNAL(info(QString)));
    connect(sites, SIGNAL(debug(QString)), this, SIGNAL(debug(QString)));
    connect(sites, SIGNAL(error(QString)), this, SIGNAL(error(QString)));

    sites->add_item("http://www.opennet.ru/");
    sites->add_item("http://www.securitylab.ru/");

    ListWidget_Link *links = new ListWidget_Link(webview, this);
    connect(links, SIGNAL(info(QString)),  this, SIGNAL(info(QString)));
    connect(links, SIGNAL(debug(QString)), this, SIGNAL(debug(QString)));
    connect(links, SIGNAL(error(QString)), this, SIGNAL(error(QString)));
    ...
    connect(webview, SIGNAL(loadStarted()), this, SLOT(started()));
    connect(webview, SIGNAL(loadFinished(bool)), this, SLOT(finished(bool)));
    connect(webview, SIGNAL(loadProgress(int)), progressBar, SLOT(setValue(int)));
    if(mw)
    {
        connect(webview, SIGNAL(titleChanged(QString)), mw, SLOT(setWindowTitle(QString)));
    ...
    cookies->load();
    webview->page()->networkAccessManager()->setCookieJar(cookies);
}
//--------------------------------------------------------------------------------
...
void MainBox::createUrlBar(void)
{
    ...
    QToolButton *btn_add_linux_moy_su = add_button(toolBar,
                                                   new QToolButton(this),
                                                   QIcon(),
                                                   "add_linux_moy_su",
                                                   "add_linux_moy_su");
    ...
    connect(btn_add_linux_moy_su, SIGNAL(clicked()), this, SLOT(add_linux_moy_su()));
}
//--------------------------------------------------------------------------------
...
{
    bool ok;
    QString text = QInputDialog::getText(this, tr("rank site"), "site:", QLineEdit::Normal, "skif.biz", &ok);
