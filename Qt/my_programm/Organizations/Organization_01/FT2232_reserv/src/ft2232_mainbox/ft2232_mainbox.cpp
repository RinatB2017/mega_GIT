...
#include "ui_ft2232_mainbox.h"
//--------------------------------------------------------------------------------
#include "mysplashscreen.hpp"
#include "doublehexeditor.hpp"
#include "mainwindow.hpp"
#include "ft2232_mainbox.hpp"
#include "defines.hpp"
#include "sleeper.h"
...
void MainBox::test_eeprom(void)
{
...
#if 0
    err = ftdi_set_eeprom_value(&ftdi, CHIP_SIZE, 0xFF);
    if(err)
    {
        switch(err)
        {
    ...
    err = ftdi_get_eeprom_buf(&ftdi, buf, 256);
    if(err)
    {
        switch(err)
        {
