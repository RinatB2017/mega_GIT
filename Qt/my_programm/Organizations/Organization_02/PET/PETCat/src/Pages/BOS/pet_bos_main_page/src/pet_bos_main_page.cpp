...
#include "ui_pet_bos_main_page.h"
#include "defines.hpp"
#include "pet.h"
//--------------------------------------------------------------------------------
#ifdef QT_DEBUG
...
#endif
//--------------------------------------------------------------------------------
PET_bos_main_page::PET_bos_main_page(QWidget *parent, bool is_demo) :
    PET_device_bos(parent),
    ui(new Ui::PET_bos_main_page),
    flag_is_demo(is_demo)
{
    init();
}
//--------------------------------------------------------------------------------
PET_bos_main_page::~PET_bos_main_page()
{
...
//--------------------------------------------------------------------------------
void PET_bos_main_page::update_widgets(void)
{
    main_refresh();
}
//--------------------------------------------------------------------------------
//--------------------------------------------------------------------------------
void PET_bos_main_page::main_refresh(void)
{
    block_this_button(true);
    QString o_name = ui->tabWidget->currentWidget()->objectName();
    emit debug(QString("PET_bos_main_page::main_refresh %1")
               .arg(o_name));

    if(o_name == "p_device_info")
    {
        f_PET_CMD_GET_DEVICE_INFO();
    }
    if(o_name == "p_net_config")
    {
        f_PET_CMD_GET_CONTROL();
        //FIXME
    }
    emit debug(QString("o_name [%1]").arg(o_name));
    block_this_button(false);
}
//--------------------------------------------------------------------------------
void PET_bos_main_page::main_apply(void)
{
    block_this_button(true);
    //TODO
    block_this_button(false);
...
//--------------------------------------------------------------------------------
void PET_bos_main_page::main_close(void)
{
    emit error("Close");
    parentWidget()->close();
}
//--------------------------------------------------------------------------------
//--------------------------------------------------------------------------------
void PET_bos_main_page::add_page_p_device_info(void)
{
...
//--------------------------------------------------------------------------------
void PET_bos_main_page::add_page_p_net_config(void)
{
...
//--------------------------------------------------------------------------------
void PET_bos_main_page::init(void)
{
    ui->setupUi(this);

    //TODO mark
    //---
    mark_spinbox("setting_name", "bos_main_page");
    mark_doublespinbox("setting_name", "bos_main_page");
    mark_combobox("setting_name", "bos_main_page");
    //---
...
//--------------------------------------------------------------------------------
void PET_bos_main_page::btn_refresh_click(void)
{
    main_refresh();
}
//--------------------------------------------------------------------------------
void PET_bos_main_page::btn_apply_click(void)
{
    main_apply();
}
//--------------------------------------------------------------------------------
void PET_bos_main_page::btn_close_click(void)
{
    main_close();
}
//--------------------------------------------------------------------------------
void PET_bos_main_page::btn_import_click(void)
{
    f_import();
}
//--------------------------------------------------------------------------------
void PET_bos_main_page::btn_export_click(void)
{
    f_export();
}
//--------------------------------------------------------------------------------
void PET_bos_main_page::all_refresh(void)
{
    f_PET_CMD_GET_DEVICE_INFO();
    //f_PET_CMD_GET_STATE();
    f_PET_CMD_GET_CONTROL();
...
//--------------------------------------------------------------------------------
void PET_bos_main_page::get_packet(QByteArray packet)
{
...
//--------------------------------------------------------------------------------
PET_RESULT PET_bos_main_page::f_PET_CMD_GET_DEVICE_INFO(void)
{
...
//--------------------------------------------------------------------------------
PET_RESULT PET_bos_main_page::f_PET_CMD_GET_CONTROL(void)
{
    emit info("Запрос параметров режима работы устройства");
    block_interface(p_net_config, true);
    //---
    pet_bos_ctrl_t ctrl;
    ...
    block_interface(p_net_config, false);
    emit info("Выполнено");
    return PET_SUCCESS;
}
//--------------------------------------------------------------------------------
PET_RESULT PET_bos_main_page::f_PET_CMD_SET_CONTROL(void)
{
...
//--------------------------------------------------------------------------------
void PET_bos_main_page::block_this_button(bool state)
{
...
//--------------------------------------------------------------------------------
void PET_bos_main_page::block_interface(QWidget *widget, bool state)
{
    if(!widget)
    {
        return;
...
//--------------------------------------------------------------------------------
bool PET_bos_main_page::check_packet(uint8_t cmd, QByteArray packet)
{
    //emit debug(QString("check_size_packet: [%1]").arg(packet.toHex().data()));
    if(packet.length() < (int)sizeof(pet_frame_t))
    {
        emit error("PET_bos_main_page::check_size_packet: packet too small");
        return false;
    }
    ...
    {
        emit error("PET_bos_main_page::check_size_packet: bad len");
        return false;
    }
...
//--------------------------------------------------------------------------------
//FIXME костыль
void PET_bos_main_page::f_import(void)
{
    PET_device_bos::f_import();
}
void PET_bos_main_page::f_export(void)
{
    PET_device_bos::f_export();
}
//--------------------------------------------------------------------------------
