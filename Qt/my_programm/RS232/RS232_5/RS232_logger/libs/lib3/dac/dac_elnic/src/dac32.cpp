...
bool DAC32::f_GET_SWITCHES(void)
{
    ...
    emit debug(QString("cmd  %1").arg(answer->header.cmd));
    emit debug(QString("cnt  %1").arg(answer->header.count_data));
    emit debug(QString("val  0x%1").arg(answer->data.values.data_0.value, 4, 16, QChar('0')));
    emit debug(QString("crc8 0x%1").arg(answer->crc8, 2, 16, QChar('0')));
...
bool DAC32::f_get_switches(uint32_t *value)
{
    ...
    *value = answer->data.values.data_0.value;
    return true;
}
//--------------------------------------------------------------------------------
...
bool DAC32::f_SET_SWITCHES(void)
{
    ...
    emit debug(QString("sw: 0x%1").arg(sw.data_0.value, 4, 16, QChar('0')));
...
bool DAC32::f_SET_SWITCHES(SWITCH values)
{
    ...
    emit debug(QString("sw: 0x%1").arg(values.data_0.value, 4, 16, QChar('0')));
...
//--------------------------------------------------------------------------------
bool DAC32::f_GET_DAC_ZERO_U0(uint16_t *value)
{
    GET_DAC_ZERO_U0_Q question;
    question.header.address = MY_ADDR;
    question.header.cmd = GET_DAC_ZERO_U0;
    question.header.count_data = 0;
    ...
    bool ok = check_answer<GET_DAC_ZERO_U0_A>(GET_DAC_ZERO_U0);
    if(!ok)
    {
        return false;
    }

    GET_DAC_ZERO_U0_A *answer = reinterpret_cast<GET_DAC_ZERO_U0_A *>(dac_data.data());
    Q_ASSERT(answer);
...
//--------------------------------------------------------------------------------
bool DAC32::f_SET_DAC_ZERO_U0(uint16_t value)
{
    SET_DAC_ZERO_Q question;
    question.header.address = MY_ADDR;
    question.header.cmd = SET_DAC_ZERO_U0;
    question.header.count_data = sizeof(question.data);
    question.data.value = value;
    ...
    bool ok = check_answer<SET_DAC_ZERO_U0_A>(SET_DAC_ZERO_U0);
    if(!ok)
    {
        return false;
    }

    SET_DAC_ZERO_U0_A *answer = reinterpret_cast<SET_DAC_ZERO_U0_A *>(dac_data.data());
    Q_ASSERT(answer);
...
//--------------------------------------------------------------------------------
bool DAC32::f_GET_DAC_ADV(uint16_t *value)
{
    GET_DAC_ADV_Q question;
    question.header.address = MY_ADDR;
    question.header.cmd = GET_DAC_ADV;
    question.header.count_data = 0;
    ...
    bool ok = check_answer<GET_DAC_ADV_A>(GET_DAC_ADV);
    if(!ok)
    {
        return false;
    }

    GET_DAC_ADV_A *answer = reinterpret_cast<GET_DAC_ADV_A *>(dac_data.data());
    Q_ASSERT(answer);
...
//--------------------------------------------------------------------------------
bool DAC32::f_SET_DAC_ADV(uint16_t value)
{
    SET_DAC_ADV_Q question;
    question.header.address = MY_ADDR;
    question.header.cmd = SET_DAC_ADV;
    question.header.count_data = sizeof(question.data);
    question.data.value = value;
    ...
    bool ok = check_answer<SET_DAC_ADV_A>(SET_DAC_ADV);
    if(!ok)
    {
        return false;
    }

    SET_DAC_ADV_A *answer = reinterpret_cast<SET_DAC_ADV_A *>(dac_data.data());
    Q_ASSERT(answer);
