...
void Calibration::init(void)
{
    ...
    connect(imitator_brg,   SIGNAL(trace(QString)),     this,   SIGNAL(trace(QString)));
    //---
...
void Calibration::refresh(void)
{
    ...
    emit info(QString("Чтение калибровочной информации БРГ (адрес %1)").arg(pet_addr));
...
void Calibration::apply(void)
{
    ...
    PET_RESULT res = PET_SUCCESS;
    emit info(QString("Запись калибровочной информации БРГ (адрес %1)").arg(pet_addr));
    pet_brg->set_pet_addr(pet_addr);
