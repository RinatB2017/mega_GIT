...
//--------------------------------------------------------------------------------
void MainWindow::f_petcat_options(int index)
{
    PETCat_optionsBox *w = new PETCat_optionsBox(this);
    if(w)
    {
        w->setObjectName("PETCat_options");
        connect(w,  SIGNAL(signal_is_shows_info(bool)),     this,   SIGNAL(signal_is_shows_info(bool)));
        connect(w,  SIGNAL(signal_is_shows_debug(bool)),    this,   SIGNAL(signal_is_shows_debug(bool)));
        connect(w,  SIGNAL(signal_is_shows_error(bool)),    this,   SIGNAL(signal_is_shows_error(bool)));
        connect(w,  SIGNAL(signal_is_shows_trace(bool)),    this,   SIGNAL(signal_is_shows_trace(bool)));

        w->set_current_index(index);    //FIXME
...
void MainWindow::f_options(void)
{
    ...
    FindPetBox *f = new FindPetBox(permitted_dev, this);   //TODO
    int btn = f->exec();
    if(btn != QDialog::Accepted)
    {
        return;
...
{
    m_file   = menuBar()->addMenu(NAME_FILE_MENU);
...
//--------------------------------------------------------------------------------
void MainWindow::add_task_menu(void)
{
    m_task = menuBar()->addMenu(NAME_TASKS_MENU);
    //---
    m_scan_menu = new QMenu(this);
    m_scan_menu->setTitle(NAME_SCAN);
    //---
    m_options = new QMenu(this);
    m_options->setTitle(NAME_OPTIONS);
    //---
    m_analize = new QMenu(this);
    m_analize->setTitle(NAME_ANALIZE);
    //---
    m_tests = new QMenu(this);
    m_tests->setTitle(NAME_TESTS);
    //---
#if (defined MKT) || (defined SIMULATOR) || (defined UPDATE_FIRMWARE)
    m_other = new QMenu(this);
    m_other->setTitle(NAME_OTHER);
#endif
    //---
    a_scan_control = new QAction(this);
    a_scan_control->setText(NAME_SCAN_CONTROL);
    a_scan_control->setIcon(QIcon(NAME_SCAN_CONTROL_ICON));
    m_scan_menu->addAction(a_scan_control);
    connect(a_scan_control, SIGNAL(triggered(bool)),    this,   SLOT(f_scan_control()));
    //---
#ifdef SCAN_HARD
    a_scan_hard = new QAction(this);
    a_scan_hard->setText(NAME_SCAN_HARD);
    a_scan_hard->setIcon(QIcon(NAME_SCAN_ICON));
    m_scan_menu->addAction(a_scan_hard);
    connect(a_scan_hard, SIGNAL(triggered(bool)),    this,   SLOT(f_scan_hard()));
#endif
    //---
    ...
    m_scan_menu->addAction(a_oscilloscope);
    connect(a_oscilloscope, SIGNAL(triggered(bool)),    this,   SLOT(f_oscilloscope()));
    //---
    a_viewer = new QAction(this);
    a_viewer->setText(NAME_VIEWER);
    a_viewer->setIcon(QIcon(NAME_VIEWER_ICON));
    m_analize->addAction(a_viewer);
    ...
    a_options = new QAction(this);
    a_options->setText(NAME_OPTIONS);
    a_options->setIcon(QIcon(NAME_OPTIONS_ICON));
    m_options->addAction(a_options);
    connect(a_options, SIGNAL(triggered(bool)),    this,   SLOT(f_options()));
    //---
    a_demo = new QAction(this);
    a_demo->setText(NAME_DEMO);
    a_demo->setIcon(QIcon(NAME_DEMO_ICON));
    m_tests->addAction(a_demo);
    connect(a_demo, SIGNAL(triggered(bool)),    this,   SLOT(f_demo()));
    //---
    a_imitator = new QAction(this);
    a_imitator->setText(NAME_IMITATOR_BRG);
    a_imitator->setIcon(QIcon(NAME_IMITATOR_BRG_ICON));
    m_tests->addAction(a_imitator);
    connect(a_imitator, SIGNAL(triggered(bool)),    this,   SLOT(f_imitator_brg()));
    //---
    a_calibration = new QAction(this);
    a_calibration->setText(NAME_CALIBRATION);
    a_calibration->setIcon(QIcon(NAME_CALIBRATION_ICON));
    m_tests->addAction(a_calibration);
    connect(a_calibration, SIGNAL(triggered(bool)),    this,   SLOT(f_calibration()));
    //---
#ifdef TEST_1
    a_test_1 = new QAction(this);
    a_test_1->setText(NAME_TEST_1);
    a_test_1->setIcon(QIcon(NAME_TEST_1_ICON));
    m_tests->addAction(a_test_1);
    connect(a_test_1, SIGNAL(triggered(bool)),    this,   SLOT(f_test_1()));
#endif
#ifdef TEST_2
    a_test_2 = new QAction(this);
    a_test_2->setText(NAME_TEST_2);
    a_test_2->setIcon(QIcon(NAME_TEST_2_ICON));
    m_tests->addAction(a_test_2);
    connect(a_test_2, SIGNAL(triggered(bool)),    this,   SLOT(f_test_2()));
#endif
    //---
#if (defined MKT) || (defined SIMULATOR) || (defined UPDATE_FIRMWARE)
    a_update_firmware = new QAction(this);
    a_update_firmware->setText(NAME_UPDATE_FIRMWARE);
    a_update_firmware->setIcon(QIcon(NAME_UPDATE_FIRMWARE_ICON));
    m_other->addAction(a_update_firmware);
    connect(a_update_firmware, SIGNAL(triggered(bool)),    this,   SLOT(f_update_firmware()));
#endif
    //---
    m_task->addMenu(m_scan_menu);
    m_task->addMenu(m_analize);
    m_task->addMenu(m_options);
    m_task->addMenu(m_tests);
#if (defined MKT) || (defined SIMULATOR) || (defined UPDATE_FIRMWARE)
    m_task->addMenu(m_other);
#endif
}
//--------------------------------------------------------------------------------
void MainWindow::add_windows_menu(void)
{
    m_windows = menuBar()->addMenu(NAME_WINDOWS_MENU);
}
//--------------------------------------------------------------------------------
void MainWindow::add_help_menu(void)
{
    m_help = menuBar()->addMenu(NAME_HELP_MENU);

    a_help = new QAction(this);
    a_help->setText(NAME_HELP_MENU);
    a_help->setToolTip(NAME_HELP_MENU);
    a_help->setStatusTip(NAME_HELP_MENU);
    a_help->setIcon(QIcon(NAME_HELP_ICON));
...
void MainWindow::create_left_dock(void)
{
    ...
    QDockWidget *dock = new QDockWidget(NAME_TASKS, this);
...
{
    ld = new LogDock("Протокол работы", this);
    ...
    connect(this,   SIGNAL(signal_is_shows_info(bool)),     ld, SLOT(set_flag_is_shows_info(bool)));
    connect(this,   SIGNAL(signal_is_shows_debug(bool)),    ld, SLOT(set_flag_is_shows_debug(bool)));
    connect(this,   SIGNAL(signal_is_shows_error(bool)),    ld, SLOT(set_flag_is_shows_error(bool)));
    connect(this,   SIGNAL(signal_is_shows_trace(bool)),    ld, SLOT(set_flag_is_shows_trace(bool)));
...
{
    nld = new LogDock("Протокол сетевого обмена", this);
    m_windows->addAction(nld->toggleViewAction());
    nld->setObjectName("net");

    connect(this,   SIGNAL(send_net_log(QString)),      this, SLOT(slot_send_net_log(QString)));
    connect(this,   SIGNAL(receive_net_log(QString)),   this, SLOT(slot_receive_net_log(QString)));

    connect(this,   SIGNAL(signal_is_shows_info(bool)),     nld, SLOT(set_flag_is_shows_info(bool)));
    connect(this,   SIGNAL(signal_is_shows_debug(bool)),    nld, SLOT(set_flag_is_shows_debug(bool)));
    connect(this,   SIGNAL(signal_is_shows_error(bool)),    nld, SLOT(set_flag_is_shows_error(bool)));
    connect(this,   SIGNAL(signal_is_shows_trace(bool)),    nld, SLOT(set_flag_is_shows_trace(bool)));

    addDockWidget(Qt::BottomDockWidgetArea, nld);
}
#endif
//--------------------------------------------------------------------------------
void MainWindow::slot_send_net_log(QString data)
{
    if(ld)
    {
        ld->errorLog(QString("send %1").arg(data));
    }
}
//--------------------------------------------------------------------------------
void MainWindow::slot_receive_net_log(QString data)
{
    if(ld)
    {
        ld->infoLog(QString("receive %1").arg(data));
    }
}
//--------------------------------------------------------------------------------
