...
bool AbstractSerialPrivate::canReadNotification()
{
    ...
    if (this->readSerialNotifierCalled) {
        if (!this->readSerialNotifierStateSet) {
            ...
            this->readSerialNotifierState = serialEngine->isReadNotificationEnabled();
            serialEngine->setReadNotificationEnabled(false);
        }
    }
    ...
    bool hasData = (this->isBuffered) ? (newBytes > 0) : (serialEngine->bytesAvailable() > 0);
    ...
    if ((!hasData) && serialEngine && (!serialEngine->isReadNotificationEnabled()))
        serialEngine->setReadNotificationEnabled(true);

    // reset the read serial notifier state if we reentered inside the
    // readyRead() connected slot.
    if (this->readSerialNotifierStateSet && serialEngine &&
        (this->readSerialNotifierState != serialEngine->isReadNotificationEnabled())) {

        serialEngine->setReadNotificationEnabled(this->readSerialNotifierState);
...
{
#if defined (Q_OS_WIN)
    if (serialEngine && serialEngine->isWriteNotificationEnabled())
        serialEngine->setWriteNotificationEnabled(false);
#endif
...
bool AbstractSerialPrivate::canLineNotification()
{
    ...
    bool newState = false;
    quint16 line = serialEngine->lineStatus();
...
{
    qint64 bytesToRead = serialEngine->bytesAvailable();
    ...
    qint64 readBytes = serialEngine->read(ptr, bytesToRead);
...
bool AbstractSerialPrivate::flush()
{
    ...
    if ((!serialEngine) || this->writeBuffer.isEmpty())
        return false;
    ...
    qint64 written = serialEngine->write(ptr, nextSize);
    if (written < 0) {
    ...
    if (this->writeBuffer.isEmpty() && serialEngine && serialEngine->isWriteNotificationEnabled())
        serialEngine->setWriteNotificationEnabled(false);
