...
void ProcessingImage::find_contours()
{
    ...
    //TODO в оригинале не было
    if(first_contour == nullptr)
    {
        return;
    }

    approx_contours = cvApproxPoly(first_contour,
                                   sizeof(CvContour),
                                   storage,
...
QVector<Matrix> ProcessingImage::get_contours_pixels()
{
    ...
    for(int t = 0; t < N; ++t)
    {
        ...
        //cv::Mat convData = cv::Mat(&data);  //TODO
        cv::Mat convData;
