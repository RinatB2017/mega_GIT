...
bool ORPrintRender::setupPrinter(ORODocument * pDocument, QPrinter * pPrinter)
{
    ...
    if(psi.isNull())
    {
        ...
        pPrinter->setPageSize(QPageSize(static_cast<QPageSize::PageSizeId>(psi.qpValue())));
    }
    else
        pPrinter->setPageSize(QPageSize(static_cast<QPageSize::PageSizeId>(psi.qpValue())));
...
bool ORPrintRender::render(ORODocument * pDocument)
{
    ...
    for(int copy = 0; copy < _printer->numCopies(); copy++)
    {
        for(int page = fromPage; page < toPage; page++)
        {
            ...
            QSize margins(_printer->paperRect(QPrinter::Point).left() - _printer->pageRect(QPrinter::Point).left(), _printer->paperRect(QPrinter::Point).top() - _printer->pageRect(QPrinter::Point).top());
...
void renderBackground(QImage & dest,
                      const QImage & bgImage,
                      const QRect & bgRect,
                      bool bgScale,
                      Qt::AspectRatioMode bgScaleMode,
                      int bgAlign,
                      unsigned int bgOpacity)
{
    QImage img = bgImage;
    if(img.isNull())
        return;
...
void renderWatermark(QImage & image,
                     const QString & wmText,
                     const QFont & wmFont,
                     const unsigned int wmOpacity,
                     double pA,
                     double pB,
                     double pC,
                     double pD)
...
void ORPrintRender::renderPage(ORODocument * pDocument,
                               int pageNb,
                               QPainter *painter,
                               qreal xDpi,
                               qreal yDpi,
...
bool ORPrintRender::exportToPDF(ORODocument * pDocument,
                                QString pdfFileName)
{
    if(!pDocument)
        return false;
